...
class NodeFactory:
    ...
    def create_node_function(
        cls,
        config: Union[Engine, Callable, str],
        ...
        from haive_core.engine.base import EngineRegistry, EngineType, Engine, InvokableEngine, NonInvokableEngine
        ...
        if isinstance(config, Engine):
            ...
            if isinstance(config, InvokableEngine):
                ...
                elif config.engine_type == EngineType.VECTOR_STORE:
                ...
                elif config.engine_type == EngineType.RETRIEVER:
                ...
                elif config.engine_type == EngineType.AGENT:
            ...
            elif isinstance(config, NonInvokableEngine):
    ...
    def create_tool_node(
        ...
        def node_function(state, config: Optional[RunnableConfig] = None):
    ...
    def create_structured_output_node(
        ...
        def node_function(state, config=None):
            ...
            try:
                ...
                return Command(
                    update={"error": f"Processing error: {str(e)}"},
                    goto=command_goto
                )
    ...
    def _create_invokable_engine_node(
        ...
        elif engine.engine_type == EngineType.VECTOR_STORE:
        ...
        elif engine.engine_type == EngineType.AGENT:
    ...
    def _create_llm_node(
        cls,
        engine: Union[AugLLMConfig, InvokableEngine],
        command_goto: Optional[str] = None,
        input_mapping: Optional[Dict[str, str]] = None,
        output_mapping: Optional[Dict[str, str]] = None,
        runnable_config: Optional[RunnableConfig] = None
        ...
        def node_function(state, config: Optional[RunnableConfig] = None):
            """LLM node function."""
            logger.debug(f"LLM node called with state")
    ...
    def _create_vectorstore_node(
        ...
        def node_function(state, config: Optional[RunnableConfig] = None):
            ...
            logger.debug(f"Vector store node called with state")
    ...
    def _create_retriever_node(
        ...
        def node_function(state, config: Optional[RunnableConfig] = None):
            ...
            logger.debug(f"Retriever node called with state")
    ...
    def _create_agent_node(
        ...
        def node_function(state, config: Optional[RunnableConfig] = None):
            """Agent node function."""
            logger.debug(f"Agent node called with state")
    ...
    def _create_non_invokable_engine_node(
        ...
        def node_function(state, config: Optional[RunnableConfig] = None):
            ...
            logger.debug(f"Non-invokable engine node called with state")
    ...
    def _create_embeddings_node(
        ...
        def node_function(state, config: Optional[RunnableConfig] = None):
            ...
            logger.debug(f"Embeddings node called with state")
    ...
    def _process_with_non_invokable(cls, instance, input_data):
        ...
        if isinstance(input_data, dict):
            ...
            if "text" in input_data:
                ...
                elif isinstance(first_value, list) and all(isinstance(x, str) for x in first_value):
    ...
    def _create_generic_engine_node(
        ...
        def node_function(state, config: Optional[RunnableConfig] = None):
            """Generic engine node."""
            logger.debug(f"Generic engine node called with state")
    ...
    @classmethod
    def _create_error_command(cls, state, error_message: str, command_goto: Optional[str] = None) -> Command:
        """
    ...
    def _merge_configs(
        ...
        override_config: Optional[RunnableConfig] = None
