...
from typing import Dict, List, Optional, Union, Tuple, Any, Type, get_type_hints, get_origin, get_args, Callable
from pydantic import BaseModel, create_model, Field
from haive_core.engine.base import Engine
from haive_core.graph.schema.StateSchemaManager import StateSchemaManager
from langchain_core.messages import BaseMessage
from typing import Annotated, Sequence, Optional as OptionalType
...
class SchemaComposer:
    ...
    def derive_schema_from_engine(engine: Engine, schema_manager=None):
        ...
        if 'messages' not in schema_manager.fields:
            try:
                ...
                logger.debug(f"Added messages field with add_messages reducer")
            except ImportError:
                ...
                logger.debug(f"Added messages field without reducer")
    ...
    @staticmethod
    def compose_schema(components: List[Union[Engine, BaseModel, Dict, Any]], name: str="ComposedSchema"):
        """
    ...
    @staticmethod
    def create_schema_for_components(components: List[Union[Engine, BaseModel, Dict, Any]], name: str="ComposedSchema"):
        """
    ...
    def derive_input_schema_for_node(
        node_component: Union[Engine, Callable], 
        ...
        if isinstance(node_component, Engine):
            try:
                ...
                logger.debug(f"Could not derive input schema from engine")
                pass
    ...
    def derive_output_schema_for_node(
        ...
        if isinstance(node_component, Engine):
            try:
                ...
                logger.debug(f"Could not derive output schema from engine")
                pass
