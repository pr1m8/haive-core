# src/haive/core/engine/agent/state_handling.py
...
def validate_with_schema(data: Dict[str, Any], schema: Type[BaseModel]) -> Union[BaseModel, Dict[str, Any]]:
    """
    ...
    Returns:
        Validated instance or original data if validation fails
    """
    if not schema:
        return data
...
def save_state_history(app: Any, state_filename: str, runnable_config: RunnableConfig) -> bool:
    """
    ...
    if not app:
        logger.error(f"Cannot save state history: No application provided")
        return False
    ...
    try:
        ...
        if not state_json:
            logger.warning(f"No state history available")
            return False
