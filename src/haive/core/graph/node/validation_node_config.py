...
def has_tool_error(message: ToolMessage) -> bool:
    ...
    if hasattr(message, "additional_kwargs") and message.additional_kwargs:
        if message.additional_kwargs.get("is_error"):
            logger.debug("[bold yellow]Tool error detected in additional_kwargs[/bold yellow]")
            return True
...
def get_tool_name(tool_call: Any) -> str:
    ...
    logger.warning("[bold yellow]Could not extract tool name from tool_call[/bold yellow]")
    return "unknown_tool"
...
def get_tool_args(tool_call: Any) -> dict[str, Any]:
    ...
    logger.warning("[bold yellow]Could not extract tool args from tool_call[/bold yellow]")
    return {}
...
class ValidationNodeConfig(NodeConfig, ToolRouteMixin):
    ...
    node_type: NodeType = Field(default=NodeType.VALIDATION, description="The type of node")
    ...
    tools: list[Any] = Field(default_factory=list, description="List of available tools (override)")
    ...
    tool_node: str = Field(default="tool_node", description="Node for executing standard tools")
    parser_node: str = Field(default="parse_output", description="Node for parsing Pydantic models")
    retriever_node: str = Field(default="retriever", description="Node for retriever tools")
    ...
    def _get_engine_from_state(self, state: StateLike) -> Any | None:
        ...
        if hasattr(state, "engines") and isinstance(state.engines, dict):
            ...
            if engine:
                ...
                logger.debug(f"    Engine has tool_routes: {hasattr(engine, 'tool_routes')}")
                return engine
        ...
        logger.error(f"[bold red]Engine '{self.engine_name}' not found anywhere[/bold red]")
        return None
    ...
    def _sync_tools_and_schemas(self, state: StateLike) -> tuple[list[Any], list[Any]]:
        ...
        if self.engine_name:
            ...
            if engine:
                ...
                engine_tools, engine_schemas = self._get_tools_and_schemas_from_engine(engine)
                ...
                for tool in original_engine_tools:
                    ...
                    if isinstance(tool, type) and hasattr(tool, "model_fields"):
                        logger.debug(f"  Filtering out Pydantic model class from tools: {tool}")
                        continue
                ...
                logger.error(f"[bold red]Could not find engine '{self.engine_name}'[/bold red]")
    ...
    def __call__(self, state: StateLike, config: ConfigLike | None = None) -> str | list[str]:
        ...
        if hasattr(state, "__dict__"):
            ...
            for attr in ["messages", "engines", "tools", "tool_routes"]:
                if hasattr(state, attr):
                    ...
                    if attr == "messages":
                        logger.debug(f"    {attr}: {len(attr_value) if attr_value else 0} messages")
                        if attr_value:
                            # Last 3 messages
                        ...
                        if isinstance(attr_value, dict):
                            ...
                            logger.debug(f"    {attr}: {type(attr_value)} - {attr_value}")
                    elif attr == "tools":
                        logger.debug(f"    {attr}: {len(attr_value) if attr_value else 0} tools")
                        if attr_value:
        ...
        if hasattr(state, "engines"):
            ...
            if isinstance(engines, dict):
                logger.debug(f"  Engines dict with {len(engines)} engines: {list(engines.keys())}")
        ...
        logger.debug(f"  Tool routes: {len(self.tool_routes) if self.tool_routes else 0}")
        ...
        if not hasattr(state, self.messages_key):
            logger.error(f"[bold red]State missing messages key:[/bold red] {self.messages_key}")
            logger.error(
        ...
        logger.debug(f"[bold blue]Last message type: {type(last_message).__name__}[/bold blue]")
        logger.debug(f"[bold blue]Last message content: {str(last_message)[:200]}...[/bold blue]")
        ...
        # Check alternative location
            ...
            logger.warning("[bold yellow]No tool calls found in last message[/bold yellow]")
        ...
        if not tool_calls:
            logger.info("[bold yellow]No tool calls found after extraction[/bold yellow]")
            return "no_tool_calls"
        ...
        for i, tool_call in enumerate(tool_calls):
            ...
            logger.debug(f"    Route: {route}")
            logger.debug(f"    Has model_fields: {hasattr(tool_or_schema, 'model_fields')}")
            logger.debug(f"    Has model_validate: {hasattr(tool_or_schema, 'model_validate')}")
            logger.debug(f"    Has invoke: {hasattr(tool_or_schema, 'invoke')}")
            logger.debug(f"    Has run: {hasattr(tool_or_schema, 'run')}")
            logger.debug(f"    Has args_schema: {hasattr(tool_or_schema, 'args_schema')}")
            ...
            if is_pydantic_model:
                logger.info(f"  [bold blue]Processing Pydantic model: {tool_name}[/bold blue]")
                ...
                try:
                    ...
                    logger.debug(f"Tool message content type: {type(tool_message.content)}")
                    ...
                    logger.exception(f"  [bold red]✗ Pydantic validation failed: {e}[/bold red]")
            ...
            try:
                ...
                logger.debug(f"    ValidationNode returned {len(validated_messages)} messages")
                ...
                logger.exception(f"[bold red]Validation exception for {tool_name}:[/bold red] {e}")
        ...
        if tool_messages_to_add:
            ...
            logger.warning("[bold yellow]No tool messages created during validation[/bold yellow]")
        ...
        logger.info(f"  Validation duration: {validation_duration.total_seconds():.3f}s")
        ...
        if has_errors and not destinations:
            ...
            logger.debug(f"  Reason: has_errors={has_errors}, destinations={destinations}")
            validation_result = "has_errors"
        # Route to appropriate destination(s)
        elif not destinations:
            logger.warning("[bold yellow]No valid destinations found - ending[/bold yellow]")
        ...
        logger.info(f"[bold green]✓ Validation result: {validation_result}[/bold green]")
