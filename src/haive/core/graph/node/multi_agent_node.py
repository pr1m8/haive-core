...
class MultiAgentNode(BaseNodeConfig[MultiAgentState, MultiAgentState]):
    ...
    """
    node_type: NodeType = Field(default=NodeType.AGENT, description='Node type for multi-agent execution')
    agent_name: str = Field(description='Name of the agent to execute (key in agents dict)')
    ...
    share_messages: bool = Field(default=True, description='Whether to share messages with agent state')
    update_mode: str = Field(default='merge', description="How to update container state: 'merge' or 'replace'")
    ...
    def __call__(self, state: MultiAgentState, config: ConfigLike | None=None) -> Command:
        ...
        try:
            ...
            state_update = {'agent_outputs': {**state.agent_outputs, self.agent_name: {'error': str(e)}}}
            return Command(update=state_update, goto=self._get_goto_node())
    ...
    def _project_state_for_agent(self, state: MultiAgentState, agent: Agent) -> dict[str, Any]:
    ...
    def _update_container_state(self, state: MultiAgentState, agent_result: Any) -> dict[str, Any]:
        ...
        state_update['agent_states'] = {**state.agent_states, self.agent_name: updated_agent_state}
...
class StateProjectionNode(BaseNodeConfig[TInput, TOutput]):
    ...
    """
    node_type: NodeType = Field(default=NodeType.TRANSFORM, description='Transform node type')
    ...
    field_mappings: dict[str, str] = Field(default_factory=dict, description='Map input fields to output fields')
    defaults: dict[str, Any] = Field(default_factory=dict, description='Default values for output fields')
...
def create_multi_agent_node(agent_name: str, name: str | None=None, **kwargs) -> MultiAgentNode:
...
def create_projection_node(input_schema: type[BaseModel], output_schema: type[BaseModel], name: str | None=None, **kwargs) -> StateProjectionNode:
    """Create a state projection node."""
    if not name:
        name = f'project_{input_schema.__name__}_to_{output_schema.__name__}'
    return StateProjectionNode(name=name, input_schema=input_schema, output_schema=output_schema, **kwargs)