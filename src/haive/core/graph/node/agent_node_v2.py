...
class AgentNodeConfig(BaseNodeConfig[TInput, TOutput]):
    ...
    agent_state_schema: type[BaseModel] | None = Field(default=None, description="Agent's expected state schema (if different from global)", exclude=True)
    shared_fields: list[str] = Field(default_factory=lambda: ['messages'], description='Fields to share between global and agent state')
    agent_fields: dict[str, str] | None = Field(default=None, description='Mapping of global_field -> agent_field names')
    output_field: str | None = Field(default=None, description="Field to store agent's primary output")
    preserve_messages: bool = Field(default=True, description='Whether to preserve message objects (not serialize)')
    track_execution: bool = Field(default=True, description='Whether to track execution in metadata')
    metadata_field: str = Field(default='agent_metadata', description='Field to store execution metadata')
        ...
        if self.agent_state_schema:
            for field_name, field_info in self.agent_state_schema.model_fields.items():
                if field_name not in ['messages']:
                    fields.append(FieldDefinition(name=field_name, field_type=field_info.annotation, default=field_info.default, description=field_info.description or f'Agent field: {field_name}', source=f'agent:{self.agent.name}'))
        return fields
    ...
    def get_default_output_fields(self) -> list[FieldDefinition]:
        ...
        if self.output_field:
            fields.append(FieldDefinition(name=self.output_field, field_type=Optional[Any], default=None, description=f'Output from agent {self.agent.name}'))
        fields.append(FieldDefinition(name=self.metadata_field, field_type=Optional[dict[str, Any]], default=None, description='Agent execution metadata'))
        return fields
    ...
    def __call__(self, state: StateLike, config: ConfigLike | None=None) -> Command:
        ...
        try:
            ...
            if self.track_execution:
                ...
                return Command(update={self.metadata_field: metadata}, goto=self._get_goto_node())
            raise
    ...
    def _track_start(self, state: StateLike) -> dict[str, Any]:
        ...
        return {'agent_name': self.agent.name, 'start_time': logger.time(), 'input_message_count': len(self._get_messages_from_state(state))}
    ...
    def _track_error(self, error: Exception) -> dict[str, Any]:
        ...
        return {'agent_name': self.agent.name, 'status': 'error', 'error_type': type(error).__name__, 'error_message': str(error)}
...
class CoordinatorNodeConfig(BaseNodeConfig[TInput, TOutput]):
    ...
    - Execution tracking
    """
    node_type: NodeType = Field(default=NodeType.COORDINATOR, description='Coordinator node type')
    agents: list[Agent] = Field(description='Agents to coordinate')
    mode: Literal['fanout', 'aggregate', 'sequence'] = Field(description='Coordination mode')
    aggregation_field: str = Field(default='agent_results', description='Field to store aggregated results')
    preserve_individual: bool = Field(default=True, description='Whether to preserve individual agent outputs')

    def get_default_input_fields(self) -> list[FieldDefinition]:
        """Coordinator needs messages and potentially agent results."""
        fields = [StandardFields.messages(use_enhanced=True)]
        if self.mode == 'aggregate':
            fields.append(FieldDefinition(name=self.aggregation_field, field_type=Optional[dict[str, Any]], default_factory=dict, description='Individual agent results to aggregate'))
        return fields
    ...
    def get_default_output_fields(self) -> list[FieldDefinition]:
        ...
        fields = [StandardFields.messages(use_enhanced=True)]
        if self.mode == 'aggregate':
            fields.append(FieldDefinition(name='aggregated_result', field_type=Optional[Any], default=None, description='Aggregated result from all agents'))
        return fields
    ...
    def _handle_fanout(self, state: StateLike, config: ConfigLike | None) -> Command:
        ...
        return Command(update={'coordination_stage': 'fanout'}, goto=self._get_goto_node())
    ...
    def _handle_aggregate(self, state: StateLike, config: ConfigLike | None) -> Command:
        ...
        return Command(update={'aggregated_result': aggregated, 'coordination_stage': 'aggregated'}, goto=self._get_goto_node())
    ...
    def _handle_sequence(self, state: StateLike, config: ConfigLike | None) -> Command:
        ...
        return Command(update={'agent_index': current_index + 1, 'coordination_stage': 'sequence'}, goto=self._get_goto_node())
...
def create_agent_node(agent: Agent, name: str | None=None, shared_fields: list[str] | None=None, **kwargs) -> AgentNodeConfig:
    """Create an agent node configuration."""
    if not name:
        name = f'agent_{agent.name}'
    return AgentNodeConfig(name=name, agent=agent, shared_fields=shared_fields or ['messages'], **kwargs)

def create_coordinator_node(agents: list[Agent], mode: Literal['fanout', 'aggregate', 'sequence'], name: str | None=None, **kwargs) -> CoordinatorNodeConfig:
