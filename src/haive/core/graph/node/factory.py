...
class NodeFactory:
    ...
    @classmethod
    def set_debug(
        cls, enabled: bool = True, rich_ui: bool = True, log_path: Optional[str] = None
    ) -> None:
        """
        ...
        if enabled:
            ...
            logger.info(
                f"Debug mode enabled for NodeFactory (rich_ui={cls.rich_debug})"
            )
        ...
        if cls.debug_log_path:
            try:
                ...
                with open(cls.debug_log_path, "a") as f:
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
                    f.write(
                        f"[{timestamp}] [DEBUG] {title}: {json.dumps(data, default=str)}\n"
                    )
            except Exception as e:
                logger.warning(f"Could not write to debug log file: {e}")
    ...
    def get_registry(cls) -> NodeTypeRegistry:
        ...
        if cls._registry is None:
            ...
            try:
                ...
                cls.debug_log(
                    f"Error getting registry from manager: {e}", level="warning"
                )
                cls._registry = NodeTypeRegistry.get_instance()
    ...
    def _create_node_function_impl(
        ...
        if debug_enabled:
            cls.debug_log(
                f"Creating node function from config: {type(config).__name__}"
            )
            ...
            if isinstance(config, NodeConfig):
                ...
                cls.debug_log(
                    f"Using Engine: {getattr(config, 'name', 'unknown')} ({type(config).__name__})"
                )
            elif callable(config):
                cls.debug_log(
                    f"Using callable: {getattr(config, '__name__', 'unnamed')}"
                )
            else:
        ...
        if debug_enabled and cls.rich_debug and RICH_AVAILABLE:
            ...
            config_table.add_row(
                "preserve_model", str(getattr(node_config, "preserve_model", True))
            )
            ...
            if isinstance(node_config.engine, Engine):
                config_table.add_row(
                    "engine",
                    f"{getattr(node_config.engine, 'name', 'unnamed')} ({node_config.engine.__class__.__name__})",
                )
                if hasattr(node_config.engine, "id"):
                    config_table.add_row(
                        "engine_id", getattr(node_config.engine, "id", "None")
                    )
                ...
                config_table.add_row(
                    "engine",
                    f"{getattr(node_config.engine, '__name__', 'callable')} (function)",
                )
            else:
            ...
            if node_config.command_goto == END:
                ...
                config_table.add_row(
                    "command_goto", f"Send to {node_config.command_goto.node}"
                )
            ...
            if node_config.input_mapping:
                mapping_str = ", ".join(
                    f"{k}->{v}" for k, v in node_config.input_mapping.items()
                )
            ...
            if node_config.output_mapping:
                mapping_str = ", ".join(
                    f"{k}->{v}" for k, v in node_config.output_mapping.items()
                )
        ...
        try:
            ...
            if debug_enabled:
                cls.debug_log(
                    f"Resolved engine: {type(engine).__name__}, id={engine_id}"
                )

            # Auto-detect async_mode if not specified
            if node_config.async_mode is None:
                # Check if engine supports async operations
                has_ainvoke = hasattr(engine, "ainvoke") and callable(engine.ainvoke)
                is_async_func = (
                    asyncio.iscoroutinefunction(engine) if callable(engine) else False
                )
                node_config.async_mode = has_ainvoke or is_async_func

                if debug_enabled:
                    cls.debug_log(
                        f"Auto-detected async_mode={node_config.async_mode} (has_ainvoke={has_ainvoke}, is_async_func={is_async_func})"
                    )
            ...
            if isinstance(engine, Engine):
                ...
                if debug_enabled:
                    cls.debug_log(
                        f"Pre-creating runnable for engine {engine.__class__.__name__}"
                    )
                ...
                try:
                    ...
                    if debug_enabled:
                        cls.debug_log(
                            f"Successfully created runnable: {type(runnable).__name__}"
                        )
                except Exception as e:
                    if debug_enabled:
        ...
        if node_config.async_mode:
            ...
            if hasattr(engine, "ainvoke") and callable(engine.ainvoke):
                ...
                if debug_enabled:
                    cls.debug_log("Using async_invokable processor type")
            elif asyncio.iscoroutinefunction(engine):
                node_type = "async"
                if debug_enabled:
                    cls.debug_log("Using async processor type")
            else:
        ...
        if progress and task:
            progress.update(
                task, description=f"[cyan]Getting processor for {node_type}"
            )
        ...
        if progress and task:
            progress.update(
                task,
                description=f"[cyan]Creating node function with {processor.__class__.__name__}",
            )
        ...
        try:
            ...
            if debug_enabled:
                cls.debug_log(
                    f"Created node function with processor {processor.__class__.__name__}"
                )
        except Exception as e:
            if debug_enabled:
            ...
            def error_func(state, runtime_config=None):
                ...
                if node_config.command_goto:
                    return Command(
                        update={"error": error_data}, goto=node_config.command_goto
                    )
        ...
        if node_config.debug:
            ...
            if debug_enabled:
                cls.debug_log(
                    f"Adding {'async' if node_config.async_mode else 'sync'} debug wrapper"
                )
        ...
        node_func.__preserve_model__ = (
            node_config.preserve_model
        )  # Store preserve_model setting
        node_func.__debug_id__ = f"node-{uuid.uuid4().hex[:8]}"

        # Update progress if available
        if progress and task:
            progress.update(
                task, description="[green]Node function created successfully"
            )
        ...
        if debug_enabled:
            ...
            if cls.rich_debug and RICH_AVAILABLE:
                ...
                metadata_table.add_row(
                    "preserve_model", str(node_config.preserve_model)
                )
    ...
    def _create_debug_wrapper(cls, node_func, config):
        ...
        def debug_wrapper(state, runtime_config=None):
            ...
            if cls.rich_debug and RICH_AVAILABLE:
                debug_console.rule(
                    f"[bold blue]Node Execution: [green]{config.name}[/green][/bold blue]"
                )
                debug_console.print(f"[bold]Execution ID:[/bold] {exec_id}")
                debug_console.print(
                    f"[bold]Node Type:[/bold] {getattr(node_func, '__node_type__', 'unknown')}"
                )
                debug_console.print(
                    f"[bold]Async Mode:[/bold] {getattr(node_func, '__async_mode__', 'unknown')}"
                )
                debug_console.print(
                    f"[bold]Preserve Model:[/bold] {getattr(config, 'preserve_model', True)}"
                )

                # Show state info
                function_logger.debug(
                    f"Node {config.name} called with state type: {type(state).__name__}"
                )
                ...
                if isinstance(state, BaseModel):
                    debug_console.print(
                        f"[bold]State is BaseModel:[/bold] {state.__class__.__name__}"
                    )
                    ...
                    debug_console.print(
                        f"[bold]State Keys:[/bold] {list(state.keys())}"
                    )
                    ...
                    for key, value in state.items():
                        if isinstance(value, list) and len(value) > 5:
                            debug_console.print(
                                f"[bold]{key}:[/bold] List with {len(value)} items (showing first 3)"
                            )
                            debug_console.print(Pretty(value[:3]))
                        elif isinstance(value, dict) and len(value) > 5:
                            debug_console.print(
                                f"[bold]{key}:[/bold] Dict with {len(value)} items (showing first 3)"
                            )
                            preview = {k: value[k] for k in list(value.keys())[:3]}
                            debug_console.print(Pretty(preview))
                        elif isinstance(value, str) and len(value) > 500:
                            debug_console.print(
                                f"[bold]{key}:[/bold] String with {len(value)} chars (showing first 500)"
                            )
                            debug_console.print(Text(value[:500] + "..."))

                # Show runtime config info if provided
                if runtime_config:
                    debug_console.print("[bold]Runtime Config:[/bold]")
            ...
            try:
                ...
                if cls.rich_debug and RICH_AVAILABLE:
                    debug_console.print(
                        f"[bold green]Execution successful[/bold green] in {execution_time:.3f}s"
                    )
                    debug_console.print(
                        f"[bold]Result type:[/bold] {type(result).__name__}"
                    )
                    ...
                    if isinstance(result, Command):
                        ...
                        if hasattr(result, "update") and result.update:
                            if isinstance(result.update, BaseModel):
                                debug_console.print(
                                    f"[bold]Command update is BaseModel:[/bold] {result.update.__class__.__name__}"
                                )
                                # Show some BaseModel details
                                if hasattr(result.update, "model_fields"):
                                    field_names = list(
                                        result.update.model_fields.keys()
                                    )
                                    debug_console.print(
                                        f"[bold]Model fields:[/bold] {field_names}"
                                    )
                                ...
                                debug_console.print(
                                    f"[bold]Command update keys:[/bold] {list(result.update.keys())}"
                                )
                                debug_console.print("[bold]Command update:[/bold]")
                                debug_console.print(Pretty(result.update))
                            else:
                                debug_console.print(
                                    f"[bold]Command update type:[/bold] {type(result.update).__name__}"
                                )
                    elif isinstance(result, Send):
                        debug_console.print(f"[bold]Send:[/bold] node={result.node}")
                        debug_console.print("[bold]Send arg:[/bold]")
                        debug_console.print(Pretty(result.arg))
                    elif isinstance(result, list) and all(
                        isinstance(item, Send) for item in result
                    ):
                        debug_console.print(
                            f"[bold]List of Send objects:[/bold] {len(result)} items"
                        )
                        for i, send in enumerate(result[:3]):  # Show first 3
                            debug_console.print(
                                f"[bold]Send {i}:[/bold] node={send.node}"
                            )
                        ...
                        debug_console.print(
                            f"[bold]Result is BaseModel:[/bold] {result.__class__.__name__}"
                        )
                        # Show some BaseModel details
                        ...
                        debug_console.print(
                            f"[bold]Result keys:[/bold] {list(result.keys())}"
                        )
                        debug_console.print("[bold]Result:[/bold]")
                    ...
                    # Standard logging
                    function_logger.debug(
                        f"Node {config.name} executed successfully in {execution_time:.3f}s"
                    )
                ...
                if cls.rich_debug and RICH_AVAILABLE:
                    debug_console.print(
                        f"[bold red]Execution failed[/bold red] in {execution_time:.3f}s"
                    )
                ...
                if config.command_goto:
                    return Command(
                        update={"error": error_data}, goto=config.command_goto
                    )
                return {"error": error_data}
    ...
    def _create_async_debug_wrapper(cls, node_func, config):
        ...
        def async_wrapper(state, runtime_config=None):
            ...
            if cls.rich_debug and RICH_AVAILABLE:
                debug_console.rule(
                    f"[bold blue]Async Node Execution: [green]{config.name}[/green][/bold blue]"
                )
                debug_console.print(f"[bold]Execution ID:[/bold] {exec_id}")
                debug_console.print(
                    f"[bold]Node Type:[/bold] {getattr(node_func, '__node_type__', 'unknown')}"
                )
                debug_console.print("[bold]Async Mode:[/bold] True")
                debug_console.print(
                    f"[bold]Preserve Model:[/bold] {getattr(config, 'preserve_model', True)}"
                )
                ...
                # Show state info
                function_logger.debug(
                    f"Async node {config.name} called with state type: {type(state).__name__}"
                )
                debug_console.print(f"[bold]State Type:[/bold] {type(state).__name__}")
            ...
            try:
                ...
                if cls.rich_debug and RICH_AVAILABLE:
                    debug_console.print(
                        "[bold yellow]Setting up async execution...[/bold yellow]"
                    )
                ...
                if is_coro_func:
                    ...
                    if cls.rich_debug and RICH_AVAILABLE:
                        debug_console.print(
                            "[bold yellow]Running coroutine function in event loop...[/bold yellow]"
                        )
                    ...
                    try:
                        ...
                        if cls.rich_debug and RICH_AVAILABLE:
                            debug_console.print(
                                "[bold yellow]Creating new event loop...[/bold yellow]"
                            )
                    ...
                    if cls.rich_debug and RICH_AVAILABLE:
                        debug_console.print(
                            "[bold yellow]Calling node function directly (should handle async internally)...[/bold yellow]"
                        )
                ...
                if cls.rich_debug and RICH_AVAILABLE:
                    debug_console.print(
                        f"[bold green]Async execution successful[/bold green] in {execution_time:.3f}s"
                    )
                    debug_console.print(
                        f"[bold]Result type:[/bold] {type(result).__name__}"
                    )
                    ...
                    # Standard logging
                    function_logger.debug(
                        f"Async node {config.name} executed successfully in {execution_time:.3f}s"
                    )
                    function_logger.debug(f"Result type: {type(result).__name__}")
                ...
                if cls.rich_debug and RICH_AVAILABLE:
                    debug_console.print(
                        f"[bold red]Async execution failed[/bold red] in {execution_time:.3f}s"
                    )
                    debug_console.print(f"[bold red]Error:[/bold red] {str(e)}")
                    debug_console.print_exception()
                else:
    ...
    def create_mapping_node(
        ...
        cls.debug_log(
            f"Items from: {item_provider}, Target: {target_node}, Item key: {item_key}"
        )
        ...
        def map_items(state):
            ...
            cls.debug_log(
                f"Mapping node called with state type: {type(state).__name__}"
            )
            ...
            if isinstance(state, BaseModel) and preserve_model:
                if hasattr(state, item_provider):
                    ...
                    cls.debug_log(
                        f"Extracted items from BaseModel attribute: {item_provider}"
                    )
                else:
                    items = []
            ...
            if cls.rich_debug and RICH_AVAILABLE and cls.debug_mode:
                ...
                if len(items) > 0:
                    ...
                    debug_console.print("[bold]Item samples:[/bold]")
            ...
            if cls.rich_debug and RICH_AVAILABLE and cls.debug_mode:
                debug_console.print(
                    f"[bold green]Created {len(result)} Send objects[/bold green]"
                )
                debug_console.print(
                    f"[bold]First Send sample:[/bold] node={result[0].node}, key={item_key}"
                )
    ...
    def create_conditional_node(
        ...
        if cls.rich_debug and RICH_AVAILABLE:
            ...
            if hasattr(condition_func, "__name__"):
                debug_console.print(
                    f"[bold]Condition function:[/bold] {condition_func.__name__}"
                )
            if hasattr(condition_func, "__module__"):
                debug_console.print(
                    f"[bold]Condition module:[/bold] {condition_func.__module__}"
                )
        ...
        def route_by_condition(state):
            ...
            cls.debug_log(
                f"Condition node called with state type: {type(state).__name__}"
            )
            ...
            try:
                ...
                if result in routes:
                    ...
                    if cls.rich_debug and RICH_AVAILABLE and cls.debug_mode:
                        debug_console.print(
                            f"[bold green]Routing to:[/bold green] {target_route}"
                        )
                ...
                if default_route:
                    ...
                    if cls.rich_debug and RICH_AVAILABLE and cls.debug_mode:
                        debug_console.print(
                            f"[bold yellow]No matching route, using default:[/bold yellow] {default_route}"
                        )
                ...
                if cls.rich_debug and RICH_AVAILABLE and cls.debug_mode:
                    debug_console.print(
                        f"[bold red]No route found for condition:[/bold red] {result}"
                    )
                    debug_console.print(
                        f"[bold red]Available routes:[/bold red] {list(routes.keys())}"
                    )
                ...
                if cls.rich_debug and RICH_AVAILABLE and cls.debug_mode:
                    debug_console.print(
                        f"[bold red]Error in condition function:[/bold red] {str(e)}"
                    )
                    debug_console.print_exception()
    ...
    def create_error_handler_node(
        ...
        def handle_error(state):
            ...
            cls.debug_log(
                f"Error handler called with state type: {type(state).__name__}"
            )
            ...
            if cls.rich_debug and RICH_AVAILABLE and cls.debug_mode:
                ...
                if isinstance(state, dict) and "error" in state:
                    ...
                    if isinstance(error_info, dict):
                        debug_console.print(
                            f"[bold red]Error:[/bold red] {error_info.get('error', 'Unknown error')}"
                        )
                        if "error_type" in error_info:
                            debug_console.print(
                                f"[bold red]Error type:[/bold red] {error_info['error_type']}"
                            )
                        if "traceback" in error_info:
                            debug_console.print("[bold red]Traceback:[/bold red]")
                            debug_console.print(
                                Text(error_info["traceback"], style="dim")
                            )
                    else:
                ...
                if isinstance(state, BaseModel) and hasattr(state, "error"):
                    error_info = state.error
                    debug_console.print(
                        f"[bold red]BaseModel Error:[/bold red] {error_info}"
                    )
            ...
            if isinstance(state, BaseModel) and preserve_model:
                ...
                try:
                    ...
                    try:
                        ...
                        cls.debug_log("Marked error as handled in BaseModel")
                    except AttributeError:
                        cls.debug_log(
                            "Could not set error_handled on BaseModel", level="warning"
                        )

                    # Show routing if rich is available
                    if cls.rich_debug and RICH_AVAILABLE and cls.debug_mode:
                        debug_console.print(
                            f"[bold green]Routing BaseModel to fallback node:[/bold green] {fallback_node}"
                        )
                        debug_console.rule()
                ...
                result["error_handled"] = True
                result["handled_at"] = datetime.now().isoformat()
                cls.debug_log("Marked error as handled in dict")

                # Show routing if rich is available
                if cls.rich_debug and RICH_AVAILABLE and cls.debug_mode:
                    debug_console.print(
                        f"[bold green]Routing to fallback node:[/bold green] {fallback_node}"
                    )
                    debug_console.rule()
