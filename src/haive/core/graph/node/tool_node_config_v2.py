...
class ToolNodeConfig(BaseNodeConfig[TInput, TOutput]):
    ...
    messages_field: str = Field(default='messages', description='Name of the messages field in input/output schema')
    tool_routes_field: str = Field(default='tool_routes', description='Name of the tool routes field in input schema')
    engines_field: str = Field(default='engines', description='Name of the engines field in input schema')
    error_field: str = Field(default='tool_error', description='Name of the error field in output schema')
    tags: list[str] | None = Field(default=None, description='Optional tags for the tool node')
    handle_tool_errors: bool | str | Callable[..., str] | tuple[type[Exception], ...] = Field(default=True, description='How to handle tool errors')
    engine_name: str | None = Field(default=None, description='Name of engine to get tools from')
    tools: list[BaseTool] | None = Field(default=None, description='Direct list of tools to use', exclude=True)
    allowed_routes: list[str] = Field(default_factory=lambda: ['langchain_tool', 'function', 'tool_node'], description='Tool routes this node should handle')
    require_tool_calls: bool = Field(default=True, description='Whether to require tool calls in the last message')
    create_error_messages: bool = Field(default=True, description='Whether to create ToolMessages for errors')
    ...
    def get_default_input_fields(self) -> list[FieldDefinition]:
        ...
        fields = [StandardFields.messages(use_enhanced=True), StandardFields.tool_routes()]
        if self.engine_name:
            fields.append(FieldDefinition(name=self.engines_field, field_type=dict[str, Any], default_factory=dict, description='Dictionary of available engines'))
        return fields

    def get_default_output_fields(self) -> list[FieldDefinition]:
        """Get default output field definitions."""
        return [StandardFields.messages(use_enhanced=True), FieldDefinition(name=self.error_field, field_type=Optional[str], default=None, description='Tool execution error message if any')]
    ...
    def __call__(self, state: StateLike, config: ConfigLike | None=None) -> Command:
        ...
        if not tools:
            ...
            return self._create_error_response(messages, 'No tools available for execution')
        ...
        if not filtered_tools:
            ...
            return self._create_error_response(messages, f'No tools match allowed routes: {self.allowed_routes}')
    ...
    def _filter_tools_by_route(self, tools: list[BaseTool], tool_routes: dict[str, str]) -> list[BaseTool]:
        ...
        for tool in tools:
            ...
            if route in self.allowed_routes:
                ...
                logger.debug(f"Excluding tool '{tool_name}' (route: {route} not in {self.allowed_routes})")
        return filtered

    def _execute_tools(self, state: StateLike, tools: list[BaseTool], messages: list[BaseMessage], config: ConfigLike | None) -> Command:
        """Execute tools and return updated state."""
        tool_node = ToolNode(tools=tools, name=self.name, tags=self.tags, handle_tool_errors=self.handle_tool_errors, messages_key=self.messages_field)
        ...
        if isinstance(result, dict) and self.messages_field in result:
            ...
            tool_msg_count = sum((1 for msg in updated_messages[len(messages):] if isinstance(msg, ToolMessage)))
            logger.info(f'Added {tool_msg_count} ToolMessages')
            return Command(update={self.messages_field: updated_messages, self.error_field: None}, goto=self._get_goto_node())
        logger.error(f'Unexpected result from ToolNode: {type(result)}')
        return self._create_error_response(messages, 'Unexpected result format from tool execution')
    ...
    def _create_error_response(self, messages: list[BaseMessage], error_msg: str) -> Command:
        ...
        if self.create_error_messages and messages:
            ...
            if self._has_tool_calls(last_message):
                ...
                for tool_call in last_message.tool_calls:
                    ...
                    tool_id = tool_call.get('id', f'call_{tool_name}') if isinstance(tool_call, dict) else tool_call.id
                    tool_msg = ToolMessage(content=f'Tool execution error: {error_msg}', name=tool_name, tool_call_id=tool_id)
...
def create_tool_node(name: str='tool_node', engine_name: str | None=None, tools: list[BaseTool] | None=None, allowed_routes: list[str] | None=None, **kwargs) -> ToolNodeConfig:
...
def create_langchain_tool_node(name: str='langchain_tools', engine_name: str | None=None, tools: list[BaseTool] | None=None, **kwargs) -> LangChainToolNode:
    """Create a tool node for LangChain tools."""
    return LangChainToolNode(name=name, engine_name=engine_name, tools=tools, **kwargs)

def create_function_tool_node(name: str='function_tools', engine_name: str | None=None, tools: list[BaseTool] | None=None, **kwargs) -> FunctionToolNode:
    """Create a tool node for function tools."""
    return FunctionToolNode(name=name, engine_name=engine_name, tools=tools, **kwargs)

def create_pydantic_tool_node(name: str='pydantic_tools', engine_name: str | None=None, tools: list[BaseTool] | None=None, **kwargs) -> PydanticToolNode:
    """Create a tool node for Pydantic model tools."""
    return PydanticToolNode(name=name, engine_name=engine_name, tools=tools, **kwargs)

def create_tool_node_from_route_filter(allowed_routes: list[str], engine_name: str, name: str | None=None, **kwargs) -> ToolNodeConfig:
    ...
    return ToolNodeConfig(name=name, allowed_routes=allowed_routes, engine_name=engine_name, **kwargs)