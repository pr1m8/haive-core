...
class AgentNodeV3Config(BaseNodeConfig[TInput, TOutput]):
    ...
    """
    node_type: NodeType = Field(default=NodeType.AGENT, description='Node type for agent execution')
    agent_name: str = Field(description="Name of agent to execute (key in container's agents dict)")
    agent: Agent | None = Field(default=None, description='Agent instance (extracted from state if not provided)')
    agent_state_field: str = Field(default='agent_states', description='Field in container holding agent states')
    agents_field: str = Field(default='agents', description='Field in container holding agent instances')
    project_state: bool = Field(default=True, description="Whether to project state to agent's expected schema")
    shared_fields: list[str] = Field(default_factory=lambda: ['messages'], description='Fields to share from container to agent')
    output_mode: str = Field(default='merge', description="How to handle outputs: 'merge', 'replace', or 'isolate'")
    update_container_state: bool = Field(default=True, description="Whether to update the container's agent_states")
    track_recompilation: bool = Field(default=True, description='Whether to track agent recompilation needs')
    ...
    def get_default_input_fields(self) -> list[FieldDefinition]:
        ...
        fields.append(FieldDefinition(name=self.agent_state_field, field_type=dict[str, dict[str, Any]], default_factory=dict, description='Agent states container'))
        fields.append(FieldDefinition(name=self.agents_field, field_type=dict[str, 'Agent'], default_factory=dict, description='Agent instances'))
        return fields
    ...
    def get_default_output_fields(self) -> list[FieldDefinition]:
        ...
        if self.update_container_state:
            fields.append(FieldDefinition(name=self.agent_state_field, field_type=dict[str, dict[str, Any]], default_factory=dict, description='Updated agent states'))
        fields.append(FieldDefinition(name='agent_outputs', field_type=dict[str, Any], default_factory=dict, description='Agent execution outputs'))
        return fields
    ...
    def __call__(self, state: StateLike, config: ConfigLike | None=None) -> Command:
        ...
        See Also:
            ...
            - :class:`haive.core.schema.prebuilt.multi_agent_state.MultiAgentState`: Container state
        """
        debug_mode = config and config.get('debug', False) or (hasattr(state, 'debug') and getattr(state, 'debug', False)) or False
        if debug_mode:
        ...
        try:
            ...
            error_update = {'agent_outputs': {self.agent_name: {'error': str(e), 'error_type': type(e).__name__}}}
    ...
    def _project_state_for_agent(self, state: StateLike, agent: Agent) -> dict[str, Any]:
    ...
    def _process_agent_output(self, result: Any, state: StateLike, agent: Agent) -> dict[str, Any]:
        ...
        if has_output_schema:
            ...
            if self.update_container_state:
                agent_states = getattr(state, self.agent_state_field, {})
                agent_state_data = {'executed': True, 'output_type': type(result).__name__, 'has_schema': True}
                state_update[self.agent_state_field] = {**agent_states, self.agent_name: agent_state_data}
        else:
            ...
            if 'messages' in result_dict:
                ...
                if self.update_container_state:
                    agent_states = getattr(state, self.agent_state_field, {})
                    state_update[self.agent_state_field] = {**agent_states, self.agent_name: {'executed': True, 'message_count': len(result_dict['messages'])}}
            else:
                if self.update_container_state:
                    agent_states = getattr(state, self.agent_state_field, {})
                    ...
                    state_update[self.agent_state_field] = {**agent_states, self.agent_name: updated_state}
                current_outputs = getattr(state, 'agent_outputs', {})
                state_update['agent_outputs'] = {**current_outputs, self.agent_name: result_dict}
    ...
    def _display_debug_info(self, state: StateLike, phase: str, state_update: dict[str, Any] | None=None) -> None:
        ...
        agent_branch = debug_tree.add(f"🤖 Agent '{self.agent_name}' State", style='bold yellow')
    ...
    def _add_global_state_info(self, branch: Tree, state: StateLike) -> None:
        ...
        if hasattr(state, 'messages'):
            ...
            for i, msg in enumerate(messages[-3:]):
                content = getattr(msg, 'content', str(msg))[:50] + '...' if len(str(getattr(msg, 'content', str(msg)))) > 50 else getattr(msg, 'content', str(msg))
    ...
    def _add_private_state_info(self, branch: Tree, state: StateLike) -> None:
        ...
        engine_fields = ['engine_name', 'model', 'temperature', 'max_tokens', 'token_usage']
        engine_branch = None
    ...
    def _add_state_update_info(self, branch: Tree, state_update: dict[str, Any]) -> None:
        ...
        for key, value in state_update.items():
            if key == 'messages':
                ...
                update_branch = branch.add(f"📊 {key}: Agent '{self.agent_name}' ({len(agent_updates)} fields)")
                for field, field_value in agent_updates.items():
                    if isinstance(field_value, list | tuple):
                        update_branch.add(f'  📋 {field}: [{len(field_value)} items]')
                    else:
                        value_str = str(field_value)[:30] + '...' if len(str(field_value)) > 30 else str(field_value)
                ...
                output_branch = branch.add(f"📤 {key}: Agent '{self.agent_name}' ({len(agent_output)} fields)")
    ...
    def _display_agent_input(self, agent_input: dict[str, Any], agent: Agent) -> None:
        ...
        input_tree = Tree(f"📥 Projected Input for '{self.agent_name}'", style='bold cyan')
        schema_info = input_tree.add('📋 Schema Information')
        if hasattr(agent, 'state_schema'):
            schema_name = getattr(agent.state_schema, '__name__', str(agent.state_schema))
            schema_info.add(f'State Schema: {schema_name}')
        if hasattr(agent, 'input_schema'):
            schema_name = getattr(agent.input_schema, '__name__', str(agent.input_schema))
...
def create_agent_node_v3(agent_name: str, agent: Agent | None=None, name: str | None=None, **kwargs) -> AgentNodeV3Config:
