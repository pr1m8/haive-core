...
class NodeConfig(BaseModel):
    ...
    debug_log_path: Optional[str] = Field(
        default=None,
        description="Path to save debug logs"
    )
    ...
    creation_stack: Optional[List[str]] = Field(
        default=None,
        description="Stack trace at creation time",
        exclude=True
    )
    ...
    def _debug_initialization(self):
        ...
        # Create debug header
        debug_console.rule(f"[bold blue]NodeConfig Initialization: [green]{self.name}[/green][/bold blue]")
        ...
        if isinstance(self.engine, Engine):
            debug_console.print(f"[bold]Engine:[/bold] {getattr(self.engine, 'name', 'unnamed')} ([cyan]{self.engine.__class__.__name__}[/cyan])")
            debug_console.print(f"[bold]Engine ID:[/bold] {getattr(self.engine, 'id', 'not set')}")
        elif isinstance(self.engine, str):
            debug_console.print(f"[bold]Engine Reference:[/bold] [yellow]{self.engine}[/yellow] (string reference)")
        elif callable(self.engine):
            debug_console.print(f"[bold]Engine:[/bold] [magenta]{getattr(self.engine, '__name__', 'unnamed')}[/magenta] (callable)")
        else:
        ...
        if self.command_goto is not None:
            if self.command_goto == END:
                debug_console.print("[bold]Control Flow:[/bold] [bright_green]→ END[/bright_green]")
            elif isinstance(self.command_goto, str):
                debug_console.print(f"[bold]Control Flow:[/bold] [bright_green]→ {self.command_goto}[/bright_green]")
            elif isinstance(self.command_goto, Send):
                debug_console.print(f"[bold]Control Flow:[/bold] [bright_green]→ Send to {self.command_goto.node}[/bright_green]")
                ...
                for item in self.command_goto:
                    if isinstance(item, Send):
                        debug_console.print(f"  [bright_green]→ Send to {item.node}[/bright_green]")
                    else:
        ...
        debug_console.print(f"  [dim]Node Type:[/dim] {self.node_type or 'auto-detect'}")
        debug_console.print(f"  [dim]Async Mode:[/dim] {self.async_mode or 'auto-detect'}")
        debug_console.print(f"  [dim]Use Direct Messages:[/dim] {self.use_direct_messages}")
    ...
    def validate_config(self):
        ...
        try:
            ...
            if self.debug:
                logger.exception(f"Error validating NodeConfig for {self.name}: {str(e)}")
                
                if self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[bold red]Error validating NodeConfig:[/bold red] {str(e)}")
            ...
            if isinstance(self.engine, Engine) and hasattr(self.engine, "engine_type"):
                ...
                if engine_type and getattr(engine_type, "value", "") == "llm":
                    ...
                    if self.engine.__class__.__name__ == "AugLLMConfig":
                        ...
                        structured_output_model = getattr(self.engine, "structured_output_model", None)
                        if structured_output_model:
                            ...
                            if not self.output_mapping:
                                ...
                                if isinstance(structured_output_model, type):
                                    model_name = structured_output_model.__name__.lower()
                                    # Set output mapping to map the model result to a field of the same name
                                    self.output_mapping = {"result": model_name}
                                    self.debug_log(f"Auto-derived output mapping for structured model: {self.output_mapping}")
                                else:
                        ...
                        if output_parser and not self.output_mapping:
                            ...
                            self.debug_log(f"Auto-derived output mapping for output parser: {self.output_mapping}")
                        ...
                        if prompt_template and hasattr(prompt_template, "input_variables"):
                            ...
                            if input_vars and "messages" not in input_vars:
                                ...
                                if not self.input_mapping and input_vars:
                                    self.input_mapping = {var: var for var in input_vars}
                                    self.debug_log(f"Auto-derived input mapping: {self.input_mapping}")
                                    return False
                                
                                # If all vars are already mapped, we don't need messages
                                if self.input_mapping and all(var in [v for _, v in self.input_mapping.items()] for var in input_vars):
                                    return False
    ...
    def resolve_engine(self, registry=None) -> Tuple[Any, Optional[str]]:
        ...
        if self.debug and self.rich_debug and RICH_AVAILABLE:
            ...
            debug_console.print(f"[bold]Original Engine Reference:[/bold] {self.engine}")
        ...
        try:
            ...
            if not isinstance(self.engine, str):
                ...
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Engine already resolved:[/green] {type(self.engine).__name__}")
                    if engine_id:
            ...
            if registry is None:
                ...
                try:
                    ...
                    if self.debug and self.rich_debug and RICH_AVAILABLE:
                        debug_console.print(f"[yellow]Using global EngineRegistry[/yellow]")
                except ImportError:
                    if self.debug and self.rich_debug and RICH_AVAILABLE:
                        debug_console.print(f"[red]No registry available[/red]")
                    return self.engine, None
            
            if self.debug and self.rich_debug and RICH_AVAILABLE:
                debug_console.print(f"[bold]Looking up engine:[/bold] {engine_name}")
                debug_console.print(f"[bold]Registry type:[/bold] {type(registry).__name__}")
            ...
            if engine:
                ...
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Found engine by ID:[/green] {type(engine).__name__}")
                    if engine_id:
                        debug_console.print(f"[green]Engine ID:[/green] {engine_id}")
            ...
            if engine:
                ...
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Found engine by name:[/green] {type(engine).__name__}")
                    if self.engine_id:
                        debug_console.print(f"[green]Engine ID:[/green] {self.engine_id}")
            ...
            if self.debug:
                ...
                if self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[bold red]Error resolving engine:[/bold red] {str(e)}")
                    debug_console.print_exception()
    ...
    def determine_node_type(self) -> str:
        ...
        if self.debug and self.rich_debug and RICH_AVAILABLE:
            ...
            debug_console.print(f"[bold]Engine:[/bold] {type(self.engine).__name__ if self.engine is not None else 'None'}")
        
        try:
            # Use explicit setting if provided
            if self.node_type:
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Using explicit node type:[/green] {self.node_type}")
            ...
            if self.async_mode:
                ...
                if hasattr(engine, "ainvoke") and callable(getattr(engine, "ainvoke")):
                    if self.debug and self.rich_debug and RICH_AVAILABLE:
                        debug_console.print(f"[green]Detected async_invokable (explicit async mode)[/green]")
                    return "async_invokable"
                
                # Check for async functions
                if asyncio.iscoroutinefunction(engine):
                    if self.debug and self.rich_debug and RICH_AVAILABLE:
                        debug_console.print(f"[green]Detected async (explicit async mode)[/green]")
                    return "async"
            
            # Standard detection logic
            if asyncio.iscoroutinefunction(engine):
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Detected async (coroutine function)[/green]")
                return "async"
                
            # Check for AsyncInvokable
            if hasattr(engine, "ainvoke") and callable(getattr(engine, "ainvoke")):
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Detected async_invokable[/green]")
                return "async_invokable"
                
            # Check for Invokable
            if hasattr(engine, "invoke") and callable(getattr(engine, "invoke")):
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Detected invokable[/green]")
                return "invokable"
            ...
            if callable(engine) and hasattr(engine, "__annotations__"):
                if "return" in engine.__annotations__:
                    ...
                    if "List[Send]" in str(return_type) or "list[Send]" in str(return_type):
                        if self.debug and self.rich_debug and RICH_AVAILABLE:
                            debug_console.print(f"[green]Detected mapping function[/green]")
                        return "mapping"
            
            # Default to "callable" for callable functions
            if callable(engine):
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Detected callable[/green]")
                return "callable"
                
            # Generic for everything else
            if self.debug and self.rich_debug and RICH_AVAILABLE:
                debug_console.print(f"[yellow]Defaulting to generic[/yellow]")
            return "generic"
        
        except Exception as e:
            if self.debug:
                logger.exception(f"Error determining node type for {self.name}: {str(e)}")
                
                if self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[bold red]Error determining node type:[/bold red] {str(e)}")
                    debug_console.print_exception()
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(self.engine, Engine):
                ...
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Serialized Engine as reference:[/green] {data['engine_ref']}")
                
            elif isinstance(self.engine, str):
                data["engine_ref"] = self.engine
                
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Serialized Engine as string:[/green] {data['engine_ref']}")
                ...
                if hasattr(self.engine, "__name__"):
                    ...
                    if self.debug and self.rich_debug and RICH_AVAILABLE:
                        debug_console.print(f"[green]Serialized Engine as function:[/green] {data['engine_ref']}")
                else:
                    data["engine_ref"] = "callable"
                    
                    if self.debug and self.rich_debug and RICH_AVAILABLE:
                        debug_console.print(f"[yellow]Serialized Engine as generic callable[/yellow]")
            ...
            if self.command_goto is END:
                ...
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Serialized command_goto as END[/green]")
                ...
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Serialized command_goto as Send object[/green]")
                ...
                if self.debug and self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[green]Serialized command_goto as list with Send objects[/green]")
            ...
            if self.debug:
                logger.exception(f"Error serializing NodeConfig for {self.name}: {str(e)}")
                
                if self.rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[bold red]Error serializing NodeConfig:[/bold red] {str(e)}")
                    debug_console.print_exception()
    ...
    def from_dict(cls, data: Dict[str, Any], registry=None) -> 'NodeConfig':
        ...
        if debug_mode and rich_debug and RICH_AVAILABLE:
            debug_console.rule(f"[bold]Deserializing NodeConfig: [green]{config_data.get('name', 'unnamed')}[/green][/bold]")
            debug_console.print(f"[bold]Input data:[/bold]")
        ...
        try:
            ...
            if "engine_ref" in config_data:
                ...
                if isinstance(ref, dict) and "id" in ref and "name" in ref:
                    ...
                    if registry:
                        ...
                        if ref["id"]:
                            ...
                            if debug_mode and rich_debug and RICH_AVAILABLE and engine:
                                debug_console.print(f"[green]Found engine by ID:[/green] {ref['id']}")
                        ...
                        if engine is None and ref["name"] and hasattr(registry, "find"):
                            ...
                            if debug_mode and rich_debug and RICH_AVAILABLE and engine:
                                debug_console.print(f"[green]Found engine by name:[/green] {ref['name']}")
                    ...
                    if engine:
                        ...
                        if debug_mode and rich_debug and RICH_AVAILABLE:
                            debug_console.print(f"[yellow]Engine not found, storing name reference:[/yellow] {ref['name']}")
                    ...
                    if ref.startswith("function:"):
                        ...
                        try:
                            ...
                            if debug_mode and rich_debug and RICH_AVAILABLE:
                                debug_console.print(f"[green]Imported function:[/green] {module_path}.{func_name}")
                        except (ImportError, AttributeError, ValueError) as e:
                            # Can't resolve - store as string
                            config_data["engine"] = ref
                            
                            if debug_mode and rich_debug and RICH_AVAILABLE:
                                debug_console.print(f"[yellow]Could not import function, storing reference:[/yellow] {ref}")
                        ...
                        if debug_mode and rich_debug and RICH_AVAILABLE:
                            debug_console.print(f"[green]Storing engine reference:[/green] {ref}")
            ...
            if "command_goto" in config_data:
                ...
                if goto == "END":
                    ...
                    if debug_mode and rich_debug and RICH_AVAILABLE:
                        debug_console.print(f"[green]Converted command_goto to END constant[/green]")
                    ...
                    if debug_mode and rich_debug and RICH_AVAILABLE:
                        debug_console.print(f"[green]Reconstructed Send object for command_goto[/green]")
                    ...
                    if debug_mode and rich_debug and RICH_AVAILABLE:
                        debug_console.print(f"[green]Reconstructed list with Send objects for command_goto[/green]")
            ...
            if debug_mode and rich_debug and RICH_AVAILABLE:
                debug_console.print(f"[bold green]Successfully created NodeConfig: {result.name}[/bold green]")
            ...
            if debug_mode:
                ...
                if rich_debug and RICH_AVAILABLE:
                    debug_console.print(f"[bold red]Error creating NodeConfig:[/bold red] {str(e)}")
                    debug_console.print_exception()
    ...
    def __str__(self) -> str:
        ...
        return f"NodeConfig(name='{self.name}', engine={engine_str}, " \
        ...
        if self.debug_log_path:
            try:
                with open(self.debug_log_path, "a") as f:
                    ...
                    f.write(f"[{timestamp}] [{level.upper()}] [{self.name}] {message}\n")
            except Exception as e:
    ...
    def debug_dict(self, title: str, data: Dict[str, Any]) -> None:
        ...
        # Log with rich if enabled
        if self.rich_debug and RICH_AVAILABLE:
            debug_console.print(f"[bold blue][{self.name}][/bold blue] [bold]{title}:[/bold]")
            debug_console.print(Pretty(data))
        ...
        if self.debug_log_path:
            try:
                with open(self.debug_log_path, "a") as f:
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
                    f.write(f"[{timestamp}] [DEBUG] [{self.name}] {title}: {json.dumps(data, default=str)}\n")
            except Exception as e:
                logger.warning(f"Could not write to debug log file: {e}")