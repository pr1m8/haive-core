...
class AgentNodeConfig(EngineNodeConfig):
    ...
    def __call__(
        ...
        logger.info(f"  Has compiled graph: {hasattr(agent, '_app') and agent._app is not None}")
        ...
        if isinstance(state, dict):
            ...
            if hasattr(state, "messages"):
                ...
                for i, msg in enumerate(actual_messages):
                    ...
                    if isinstance(msg, dict):
                        ...
                        if "tool_call_id" in msg:
                            logger.info(f"    Dict has tool_call_id: {msg['tool_call_id']}")
                        else:
                ...
                logger.info(f"  Extracted {len(actual_messages)} actual message objects")

                # Log message types for debugging
                for i, msg in enumerate(actual_messages):
                    logger.info(f"    Message {i}: {type(msg).__name__}")
                    if isinstance(msg, ToolMessage):
                        logger.info(f"      ToolMessage name: {getattr(msg, 'name', 'None')}")
                        logger.info(
        ...
        try:
            ...
            if hasattr(agent, "state_schema") and agent.state_schema:
                logger.info(f"  Using agent's own state schema: {agent.state_schema.__name__}")
                ...
                for field_name, _field_info in agent.state_schema.model_fields.items():
                    if field_name in state:
                        ...
                        logger.debug(f"    Extracted field '{field_name}' for agent state")
                ...
                if (
                    ...
                    if state.get("engines"):
                        ...
                        logger.debug(f"    Using agent's own engines: {list(agent.engines.keys())}")
                    else:
                ...
                try:
                    ...
                    if agent_input.get("tools"):
                        ...
                        logger.info(f"    Serialized {len(serialized_tools)} tools for agent input")

                    logger.info("  Created agent-specific state with agent's own tools/schemas")

                except Exception as e:
                    logger.warning(f"  Could not create agent-specific state: {e}")
                    logger.warning("  Falling back to prepared input from multi-agent state")
            ...
            try:
                ...
                if hasattr(agent, "_app") and agent._app:
                    ...
                    logger.info(f"  About to invoke agent with input type: {type(agent_input)}")
                    logger.info(f"  Input is dict: {isinstance(agent_input, dict)}")
                    logger.info(f"  Input is BaseModel: {isinstance(agent_input, BaseModel)}")
                    if isinstance(agent_input, dict) and "engines" in agent_input:
                        logger.info(f"  Engines in input: {list(agent_input['engines'].keys())}")
                ...
                if original_tools is not None and hasattr(agent, "engine") and agent.engine:
                    ...
                    if original_tool_routes is not None and hasattr(agent.engine, "tool_routes"):
    ...
    def _prepare_agent_input(self, state: dict[str, Any], agent: "Agent") -> dict[str, Any]:
        ...
        if hasattr(agent, "input_schema") and agent.input_schema:
            ...
            logger.debug(f"  Input schema fields: {list(agent.input_schema.model_fields.keys())}")
    ...
    def _process_agent_output(
        ...
        if isinstance(result, dict):
            ...
            if hasattr(result, "messages") and result.messages:
                ...
                logger.info("STATE UPDATE: Setting messages to actual BaseMessage objects")
                for i, msg in enumerate(result.messages):
