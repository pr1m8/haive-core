...
def extract_input(state: Any, config: NodeConfig) -> Any:
    ...
    if is_base_model:
        ...
        logger.debug(f"State keys: {list(state.keys()) if isinstance(state, dict) else 'non-dict state'}")
    ...
    is_retriever = (hasattr(config.engine, "engine_type") and 
                   getattr(config.engine, "engine_type", None) == EngineType.RETRIEVER)
    if is_retriever:
    ...
    if config.input_mapping:
        ...
        for state_key, input_key in config.input_mapping.items():
            ...
            if is_base_model:
                if hasattr(state, state_key):
                    ...
                    logger.debug(f"Mapped BaseModel attribute {state_key} → {input_key}")
                else:
        ...
        if is_retriever and "query" in mapped_input and len(mapped_input) == 1:
            logger.debug(f"Simplified retriever input to just query string: {mapped_input['query']}")
        ...
        if mapped_input:
            ...
            if len(config.input_mapping) == 1 and len(mapped_input) == 1:
                single_value = list(mapped_input.values())[0]
                logger.debug(f"Returning single mapped value: {type(single_value).__name__}")
                return single_value
    ...
    if is_retriever:
        ...
        if is_base_model and hasattr(state, "query"):
            logger.debug(f"No mapping for retriever, using direct query from BaseModel")
            return getattr(state, "query")
        # From dict
        elif isinstance(state, dict) and "query" in state:
            logger.debug(f"No mapping for retriever, using direct query: {state['query']}")
            return state["query"]
    ...
    if config.use_direct_messages:
        ...
        if is_base_model and hasattr(state, "messages"):
            ...
            logger.debug(f"Using direct messages from BaseModel (count: {len(messages) if messages else 0})")
            return messages
        # From dict
    ...
    if config.input_mapping and len(config.input_mapping) > 0:
        ...
        if is_base_model:
            available_attrs = [attr for attr in dir(state) if not attr.startswith('_') and not callable(getattr(state, attr))]
            missing_attrs = [k for k in config.input_mapping.keys() if k not in available_attrs]
            if missing_attrs:
                logger.error(f"BaseModel is missing these attributes defined in mapping: {missing_attrs}")
                logger.error(f"Available attributes: {available_attrs[:10]}{'...' if len(available_attrs) > 10 else ''}")
...
def process_output(result: Any, config: NodeConfig, original_state: Any) -> Any:
    ...
    if (isinstance(result, BaseModel) and 
        getattr(config, 'preserve_model', True) and 
        not config.output_mapping and 
        not is_retriever):
        logger.debug(f"Preserving BaseModel result directly: {result.__class__.__name__}")
        return result
    ...
    if is_original_model and getattr(config, 'preserve_model', True):
        ...
        if config.preserve_state and isinstance(original_state, dict):
            ...
            logger.debug(f"Preserving original state dict")
    ...
    if isinstance(result, dict):
                ...
                for output_key, state_key in config.output_mapping.items():
                    if output_key in result:
                        try:
                            ...
                            logger.debug(f"Mapped {output_key} → {state_key} in BaseModel")
                        except AttributeError as e:
                            logger.error(f"Cannot set attribute {state_key} on BaseModel: {e}")
            else:
                    ...
                    if output_key in result:
                        ...
                        if is_retriever and output_key == "documents" and "documents" not in result:
                            ...
                            if isinstance(result, list) and all(hasattr(doc, "page_content") for doc in result):
                                ...
                                logger.debug(f"Mapped document list directly to {state_key}")
        else:
            # No mapping
            if is_original_model and updates is not None:
        ...
        if is_original_model and updates is not None:
            ...
            if config.output_mapping:
                logger.debug(f"Applying output mapping to BaseModel")
                for output_key, state_key in config.output_mapping.items():
                ...
                try:
                    ...
                    logger.debug(f"Set BaseModel attribute '{model_name}' to result model")
                except AttributeError:
            ...
            if config.output_mapping:
                ...
                logger.debug(f"Added all model fields to state")
        ...
        if isinstance(result, BaseMessage):
            ...
            if is_original_model and updates is not None:
                if hasattr(updates, "messages") and isinstance(getattr(updates, "messages", None), list):
                    messages = getattr(updates, "messages")
                    messages.append(result)
                    logger.debug(f"Added message to existing messages list (now {len(messages)})")
                else:
                    try:
                        setattr(updates, "messages", [result])
                        logger.debug(f"Created new messages list in BaseModel")
                    except AttributeError:
                ...
                if config.extract_content:
                    try:
                        setattr(updates, "content", result.content)
                        logger.debug(f"Extracted content from message to BaseModel")
                    except AttributeError:
        ...
        if result and all(hasattr(item, "page_content") for item in result):
            logger.debug(f"Result is a list of Document-like objects (count: {len(result)})")
            ...
            if is_original_model and updates is not None:
                try:
                    ...
                    logger.warning(f"Could not set documents on BaseModel attribute: {output_key}")
            else:
                # For dictionary state
                updates[output_key] = result
            ...
            if is_original_model and updates is not None:
                try:
                    ...
                    logger.debug(f"Set list result in BaseModel attribute: result")
                except AttributeError:
                    logger.warning(f"Could not set list result on BaseModel")
            else:
                # For dictionary state
                updates["result"] = result
                logger.debug(f"Stored list in 'result'")
    else:
        ...
        if is_original_model and updates is not None:
            try:
                setattr(updates, "result", result)
                logger.debug(f"Set result in BaseModel attribute: result")
            except AttributeError:
                logger.warning(f"Could not set result on BaseModel")
        else:
            # For dictionary state
            updates["result"] = result
            logger.debug(f"Stored as 'result'")
...
def handle_command_pattern(result: Any, config: NodeConfig) -> Any:
    ...
    if isinstance(result, BaseModel) and preserve_model and config.command_goto is not None:
        logger.debug(f"Wrapping preserved BaseModel in Command with goto: {config.command_goto}")
    ...
    def create_node_function(self, engine: Any, config: NodeConfig) -> Callable:
        ...
        def node_function(state, runtime_config=None):
            ...
            try:
                ...
                is_llm = hasattr(engine, "engine_type") and getattr(engine, "engine_type", None) == EngineType.LLM
                if is_llm:
                    logger.debug("Detected LLM engine, ensuring proper message formatting")
                    
                    # If input_data is a dict with 'messages' key, extract the messages
                    if isinstance(input_data, dict) and 'messages' in input_data:
                        input_data = input_data['messages']
                        logger.debug(f"Extracted messages list from input dict")
            ...
            try:
                ...
                try:
                    if hasattr(engine, "ainvoke") and callable(getattr(engine, "ainvoke")):
                        ...
                        result = loop.run_until_complete(engine.ainvoke(input_data, merged_config))
                    else:
                        # Fall back to synchronous invoke
                        logger.debug("Falling back to synchronous invoke")
                        result = engine.invoke(input_data, merged_config)
                        
                    logger.debug(f"Async engine returned result of type: {type(result).__name__}")
                    ...
                    if hasattr(engine, "invoke") and callable(getattr(engine, "invoke")):
                        logger.warning(f"Attempting fallback to sync invoke after async error")
                        result = engine.invoke(input_data, merged_config)
