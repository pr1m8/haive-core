...
class MermaidVisualizer:
    ...
    def generate_mermaid(self, include_stats: bool = True) -> str:
        ...
        mermaid_lines.append(
            f"  style START {
                MermaidStyle.NODE_STYLES['start']}"
        )
        ...
        for name, _node_config in getattr(self.graph, "nodes", {}).items():
            ...
            if node_status == NodeStatus.UNREACHABLE:
                mermaid_lines.append(
                    f"  style {node_id} {
                        MermaidStyle.NODE_STYLES['unreachable']}"
                )
        ...
        for edge in getattr(self.graph, "edges", []):
            ...
            if edge_type == "conditional":
                ...
                condition_name = ""
                if (
                    hasattr(edge, "condition")
                    and edge.condition
                    and hasattr(edge.condition, "__name__")
                ):
                    condition_name = f" |{edge.condition.__name__}|"

                # Check for condition key in metadata
                if (
                    hasattr(edge, "metadata")
                    and edge.metadata
        ...
        try:
            ...
            if open_browser:
                try:
                    ...
                    logger.warning(f"Failed to open browser: {e!s}")
        ...
        try:
            ...
            if open_image:
                try:
                    ...
                    logger.warning(f"Failed to open image: {e!s}")

            return output_file

        except Exception as e:
            logger.exception(f"Error rendering PNG: {e!s}")
            raise
...
def replace_visualization_method() -> Any:
    ...
    try:
        ...
        logger.warning(f"Could not replace visualization method: {e!s}")
        return False
...
def generate_mermaid_diagram(
    ...
    """
    return visualize_graph(
        graph=graph, output_file=output_file, open_browser=open_result, format=format
    )
