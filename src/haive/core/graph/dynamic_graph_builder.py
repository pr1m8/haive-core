...
class DynamicGraph:
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to add node to StateGraph: {str(sg_error)}"
                ) from sg_error
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to add mapping node: {str(sg_error)}"
                ) from sg_error
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to add conditional node: {str(sg_error)}"
                ) from sg_error
    ...
    def add_error_handler(
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to add error handler: {str(sg_error)}"
                ) from sg_error
    ...
    def add_edge(
        self, from_node: str, to_node: Union[str, Literal["END"]]
    ) -> "DynamicGraph":
        """
    ...
    def add_conditional_edges(
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to add conditional edges: {str(sg_error)}"
                ) from sg_error
            ...
            tb = traceback.format_exc()
            logger.error(
                f"Error adding conditional edges from {from_node}: {str(e)}\n{tb}"
            )
    ...
    def apply_pattern(self, pattern_name: str, **kwargs) -> "DynamicGraph":
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to apply pattern {pattern_name}: {str(e)}"
                ) from e
    ...
    def debug_node(self, node_name: str, enable: bool = True) -> "DynamicGraph":
        ...
        try:
            ...
            tb = traceback.format_exc()
            logger.error(
                f"Error {'enabling' if enable else 'disabling'} debug for node {node_name}: {str(e)}\n{tb}"
            )
            ...
            try:
                ...
                if self.visualize and VISUALIZATION_AVAILABLE:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Error creating visualization: {str(viz_error)}"
                        )
                ...
                raise ValueError(
                    f"Failed to compile graph: {str(compile_error)}"
                ) from compile_error
    ...
    def debug_graph(self):
        ...
        # Check for END edges
        end_edges = [
            edge for edge in self.edges if edge.target in ["END", "__end__", str(END)]
        ]
        if not end_edges:
        ...
        if VISUALIZATION_AVAILABLE:
            try:
                ...
                os.makedirs(output_dir, exist_ok=True)
                output_file = os.path.join(
                    output_dir, f"{self.name.replace(' ', '_')}_{timestamp}.html"
                )

                print(f"\nGenerating debug visualization: {output_file}")
                visualize_graph(graph=self, output_file=output_file, open_browser=False)
