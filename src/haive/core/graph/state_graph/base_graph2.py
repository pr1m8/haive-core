...
class BaseGraph(BaseModel, ValidationMixin):
    ...
    def needs_recompile_for_interrupts(
        self, interrupt_before: list | None = None, interrupt_after: list | None = None
    ) -> bool:
    ...
    def set_conditional_entry(
        ...
        if false_node and (
            not any((src == START and dst == false_node for src, dst in self.edges))
        ):
            ...
            if not create_missing_nodes:
                for dest_name in branch.destinations.values():
                    if dest_name != END and dest_name not in self.nodes:
                        raise ValueError(
                            f"Destination node '{dest_name}' not found in graph. Use create_missing_nodes=True to create it automatically."
                        )
            self.branches[branch.id] = branch
        ...
        if not create_missing_nodes:
            for dest_key, dest_name in destination_map.items():
                if dest_name not in (END, "continue") and dest_name not in self.nodes:
                    raise ValueError(
                        f"Destination node '{dest_name}' for condition '{dest_key}' not found in graph. Use create_missing_nodes=True to create it automatically."
                    )
        ...
        if debug:
            ...
            if hasattr(self, "nodes"):
                ...
                if self.nodes:
                    ...
                    for name, node in self.nodes.items():
                        logger.debug(
                            f"Node '{name}' - Type: {(type(node).__name__ if node else 'None')}, Has graph: {(hasattr(node, 'graph') if node else False)}, Graph type: {(type(node.graph).__name__ if node and hasattr(node, 'graph') and node.graph else None)}"
                        )
