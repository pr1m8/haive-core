...
from haive.core.models.embeddings.base import BaseEmbeddingConfig, HuggingFaceEmbeddingConfig
...
class VectorStoreConfig(BaseModel):
    ...
    @classmethod
    def create_vs_config_from_documents(cls, documents: List[Document], embedding_model:BaseEmbeddingConfig=HuggingFaceEmbeddingConfig(model="sentence-transformers/all-mpnet-base-v2"), **kwargs) -> "VectorStoreConfig":
        """Create a VectorStoreConfig from a list of documents."""
        config = cls(documents=documents, embedding_model=embedding_model, **kwargs)
        return config
    @classmethod
    def create_vs_from_documents(cls, documents: List[Document], embedding_model: BaseEmbeddingConfig=HuggingFaceEmbeddingConfig(model="sentence-transformers/all-mpnet-base-v2"), **kwargs) -> "VectorStoreConfig":
...
def create_vs_config_from_documents(documents: List[Document], embedding_model: BaseEmbeddingConfig=HuggingFaceEmbeddingConfig(model="sentence-transformers/all-mpnet-base-v2"), **kwargs) -> VectorStoreConfig:
    return VectorStoreConfig.create_vs_config_from_documents(documents, embedding_model, **kwargs)
def create_vs_from_documents(documents: List[Document], embedding_model: BaseEmbeddingConfig=HuggingFaceEmbeddingConfig(model="sentence-transformers/all-mpnet-base-v2"), **kwargs) -> VectorStore:
    return VectorStoreConfig.create_vs_from_documents(documents, embedding_model, **kwargs)
def create_retriever_from_documents(documents: List[Document], embedding_model: BaseEmbeddingConfig=HuggingFaceEmbeddingConfig(model="sentence-transformers/all-mpnet-base-v2"), **kwargs) -> BaseRetriever:
    return VectorStoreConfig.create_vs_from_documents(documents, embedding_model, **kwargs).create_retriever()
