...
class StateSchema(BaseModel):
    ...
    # __reducer_fields__ is intentionally not part of the instance properties
    # because it causes serialization issues
    ...
    def apply_reducers(self, other: Union[Dict[str, Any], 'StateSchema']) -> 'StateSchema':
        ...
        # Get reducer registry - global registry for reducer functions by name
        reducer_registry = self._get_reducer_registry()
            
        # Apply simple assignment for non-reducer fields
        for key, value in data.items():
            # Check if key has a serializable reducer name
            if key in self.__class__.__serializable_reducers__:
                reducer_name = self.__class__.__serializable_reducers__[key]
                
                # Look up the reducer function from the registry
                if reducer_name in reducer_registry:
                    reducer = reducer_registry[reducer_name]
                    
                    # Apply the reducer function if the field exists
                    if hasattr(self, key):
                    ...
                    # No reducer found in registry, use simple assignment
                    setattr(self, key, value)
            else:
            ...
            for key, value in command.update.items():
                if key in self.__class__.__serializable_reducers__:
                    ...
                    if reducer_name in reducer_registry:
                        ...
                        # No reducer found, use direct assignment
                        setattr(self, key, value)
                else:
