...
"""
from typing import Any, Callable, Dict, Generic, Optional, Type, TypeVar, Union, Tuple, Annotated, get_origin, get_args
import inspect
import logging
...
class FieldDefinition(Generic[TField, TReducer]):
    ...
    def _extract_field_kwargs(self) -> Dict[str, Any]:
        ...
        field_params = [
            ...
            "json_schema_extra", "deprecated", "validation_alias",
            "serialization_alias", "validation_alias_priority",
            "serialization_alias_priority", "gt", "ge", "lt", "le",
            ...
            if len(args) > 1:
                ...
                for meta in args[1:]:
                    if callable(meta) and not isinstance(meta, type):
                        ...
                        if 'output_from' in meta and isinstance(meta['output_from'], list):
