...
class PatternConfig(BaseModel):
    ...
    parameters: dict[str, Any] = Field(default_factory=dict, description='Parameters for pattern application')
    order: int | None = Field(default=None, description='Order to apply pattern (lower numbers first)')
    condition: str | None = Field(default=None, description='Condition for pattern application')
    ...
    def merge_with(self, other: 'PatternConfig') -> 'PatternConfig':
        ...
        return PatternConfig(name=self.name, parameters=merged_params, order=other.order if other.order is not None else self.order, condition=other.condition if other.condition is not None else self.condition, enabled=other.enabled, metadata={**self.metadata, **other.metadata})
...
class AgentConfig(InvokableEngine[TIn, TOut], Generic[TIn, TOut, TState]):
    ...
    expected_protocols: ClassVar[list[type]] = [AgentProtocol, StreamingAgentProtocol, PersistentAgentProtocol, VisualizationAgentProtocol, ExtensibilityAgentProtocol]
    ...
    node_configs: dict[str, NodeConfig] = Field(default_factory=dict, description='Node configurations for explicit workflow definition')
    patterns: list[PatternConfig] = Field(default_factory=list, description='Patterns to apply to this agent')
    pattern_parameters: dict[str, dict[str, Any]] = Field(default_factory=dict, description='Global parameters for patterns by name')
    default_patterns: list[str | dict[str, Any]] = Field(default_factory=list, description='Patterns to apply by default during initialization')
    ...
    runnable_config: RunnableConfig = Field(default={'configurable': {'thread_id': str(uuid.uuid4()), 'recursion_limit': 200}})
    add_store: bool = Field(default=False)
    agent_settings: dict[str, Any] = Field(default_factory=dict)
    subagents: dict[str, 'AgentConfig'] = Field(default_factory=dict, description='Subagents for recursive composition')
    version: str = Field(default='1.0.0', description='Version of this agent configuration')
    metadata: dict[str, Any] = Field(default_factory=dict, description='Additional metadata for this agent')
    persistence: CheckpointerConfig | None = Field(default_factory=lambda: PostgresCheckpointerConfig(connection_string=os.getenv('POSTGRES_CONNECTION_STRING')) if POSTGRES_AVAILABLE and os.getenv('POSTGRES_CONNECTION_STRING') else PostgresCheckpointerConfig() if POSTGRES_AVAILABLE else MemoryCheckpointerConfig(), description='Persistence configuration for state checkpointing')
    checkpoint_mode: str = Field(default='sync', description="Checkpoint mode: 'sync', 'async', or 'none'")
    model_config = {'arbitrary_types_allowed': True}
    ...
    def get_input_fields(self) -> dict[str, tuple[type, Any]]:
        ...
        if self.input_schema is not None:
            return {name: (field_info.annotation, field_info.default) for name, field_info in self.input_schema.model_fields.items()}
        if hasattr(self, 'state_schema') and self.state_schema is not None:
            return {name: (field_info.annotation, field_info.default) for name, field_info in self.state_schema.model_fields.items()}
        return {}
    ...
    def get_output_fields(self) -> dict[str, tuple[type, Any]]:
        ...
        if self.output_schema is not None:
            return {name: (field_info.annotation, field_info.default) for name, field_info in self.output_schema.model_fields.items()}
        if hasattr(self, 'state_schema') and self.state_schema is not None:
            return {name: (field_info.annotation, field_info.default) for name, field_info in self.state_schema.model_fields.items()}
        return {}
    ...
    def _generate_schema_without_caching(self) -> type[BaseModel]:
        ...
        schema = SchemaComposer.from_components(components=all_components, name=schema_name)
            ...
            for pattern_config in self.patterns:
                ...
                if pattern:
                    for req in pattern.metadata.get('required_components', []):
                        ...
                        elif component_type == 'retriever' and (not any((getattr(c, 'engine_type', None) == EngineType.RETRIEVER for c in [self.engine, *list(self.engines.values())]))):
                            try:
                ...
                if pattern:
                    ...
                    if pattern_type == 'retrieval':
                        if not manager.has_field('context'):
                            manager.add_field('context', list[dict[str, Any]], default_factory=list)
                    elif pattern_type == 'agent':
                        if not manager.has_field('tools'):
                            manager.add_field('tools', list[dict[str, Any]], default_factory=list)
        except ImportError:
    ...
    def derive_input_schema(self) -> type[BaseModel]:
        ...
        if self.engine:
            all_components.append(self.engine)
        all_components.extend(self.engines.values())
        schema = SchemaComposer.compose_input_schema(components=all_components, name=f'{self.name}Input')
        self._input_schema_instance = schema
        return schema
    ...
    def derive_output_schema(self) -> type[BaseModel]:
        ...
        if self.engine:
            all_components.append(self.engine)
        all_components.extend(self.engines.values())
        schema = SchemaComposer.compose_output_schema(components=all_components, name=f'{self.name}Output')
        self._output_schema_instance = schema
        return schema
    ...
    def create_runnable(self, runnable_config: RunnableConfig | None=None) -> Any:
        ...
        if runnable_config:
            merged_config = RunnableConfigManager.merge(self.runnable_config, runnable_config)
    ...
    def extract_params(self) -> dict[str, Any]:
        ...
        for field_name in fields:
            if field_name.startswith('_') or field_name in ['input_schema', 'output_schema', 'id', 'name', 'engine_type']:
                continue
    ...
    def use_pattern(self, pattern_name: str, parameters: dict[str, Any] | None=None, order: int | None=None, condition: str | None=None, enabled: bool=True) -> 'AgentConfig':
        ...
        for pattern in self.patterns:
            if pattern.name == pattern_name:
                ...
                break
        if existing_pattern:
            new_pattern = PatternConfig(name=pattern_name, parameters=parameters or {}, order=order, condition=condition, enabled=enabled)
            self.patterns.remove(existing_pattern)
            self.patterns.append(existing_pattern.merge_with(new_pattern))
        else:
            self.patterns.append(PatternConfig(name=pattern_name, parameters=parameters or {}, order=order, condition=condition, enabled=enabled))
        self._invalidate_schema_caches()
        return self
    ...
    def get_pattern_order(self) -> list[str]:
        ...
        """
        sorted_patterns = sorted(self.patterns, key=lambda p: (p.order is None, p.order or 999999))
