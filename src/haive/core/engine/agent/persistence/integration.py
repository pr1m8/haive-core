...
def create_persistence_manager(persistence_config: Optional[Union[Dict[str, Any], CheckpointerConfig]] = None) -> PersistenceManager:
    """
...
async def aprepare_agent_run(
    ...
    **kwargs
    ...
    logger.debug(f"Asynchronously prepared for agent run with thread_id={current_thread_id}")
    return config, current_thread_id
