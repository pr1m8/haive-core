...
class Agent(Generic[TConfig], ABC):
    ...
    def _setup_checkpointer(self):
        ...
        if not hasattr(self.config, "persistence") or self.config.persistence is None:
            ...
            if POSTGRES_AVAILABLE:
                try:
                    from haive.core.engine.agent.persistence import PostgresCheckpointerConfig
                    # Use default values
    ...
    def visualize_graph(self, output_dir=None, output_name=None) -> None:
        """
        Generate and save a visualization of the graph.
        
        Args:
            output_dir: Directory where the graph image should be saved.
                        If None, uses default directory.
            output_name: Filename for the graph image.
                        If None, uses default name based on agent name.
        """
        try:
            if not self.graph or not self.app:
