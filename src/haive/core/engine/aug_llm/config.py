...
def _get_augllm_base_classes():
    ...
    from haive.core.common.mixins.structured_output_mixin import StructuredOutputMixin
    from haive.core.common.mixins.tool_route_mixin import ToolRouteMixin
    return (ToolRouteMixin, StructuredOutputMixin, InvokableEngine[Union[str, dict[str, Any], list[BaseMessage]], Union[BaseMessage, dict[str, Any]]])
...
class AugLLMConfig(*_get_augllm_base_classes()):
    ...
    llm_config: LLMConfig = Field(default_factory=lambda: AzureLLMConfig(model='gpt-4o') if AzureLLMConfig else None, description='LLM provider configuration')
    prompt_template: BasePromptTemplate | None = Field(default_factory=lambda: ChatPromptTemplate.from_messages([('system', 'You are a helpful assistant.'), ('placeholder', '{messages}')]), description='Prompt template for the LLM')
    system_message: str | None = Field(default=None, description='System message for chat models')
    messages_placeholder_name: str = Field(default='messages', description='Name of the messages placeholder in chat templates')
    add_messages_placeholder: bool = Field(default=True, description='Whether to automatically add MessagesPlaceholder')
    force_messages_optional: bool = Field(default=True, description='Force messages placeholder to be optional')
    examples: list[dict[str, Any]] | None = Field(default=None, description='Examples for few-shot prompting')
    example_prompt: PromptTemplate | None = Field(default=None, description='Template for formatting few-shot examples')
    prefix: str | None = Field(default=None, description='Text before examples in few-shot prompting')
    suffix: str | None = Field(default=None, description='Text after examples in few-shot prompting')
    example_separator: str = Field(default='\n\n', description='Separator between examples in few-shot prompting')
    input_variables: list[str] | None = Field(default=None, description='Input variables for the prompt template')
    schemas: Sequence[type[BaseTool] | type[BaseModel] | Callable | StructuredTool | BaseModel] = Field(default_factory=list, description='Schemas for tools')
    pydantic_tools: list[type[BaseModel]] = Field(default_factory=list, description='Pydantic models for tool schemas')
    use_tool_for_format_instructions: bool = Field(default=False, description='Use a single tool Pydantic model for format instructions')
    tool_is_base_model: bool = Field(default=False, description='Whether a tool is a BaseModel type (detected automatically)')
    force_tool_use: bool = Field(default=False, description='Whether to force the LLM to use a tool (any tool)')
    force_tool_choice: bool | str | list[str] | None = Field(default=None, description='Force specific tool(s) to be used')
    tool_choice_mode: ToolChoiceMode = Field(default='auto', description='Tool choice mode to use for binding tools')
    structured_output_model: type[BaseModel] | None = Field(default=None, description='Pydantic model for structured output')
    structured_output_version: StructuredOutputVersion | None = Field(default=None, description='Version of structured output handling: v1 (traditional), v2 (tool-based), None (disabled)')
    if TYPE_CHECKING:
        output_parser: BaseOutputParser | None = Field(default=None, description='Parser for LLM output', exclude=True)
    else:
        output_parser: Any | None = Field(default=None, description='Parser for LLM output', exclude=True)
    parse_raw_output: bool = Field(default=False, description='Force parsing raw output even with structured output model')
    include_format_instructions: bool = Field(default=True, description='Whether to include format instructions in the prompt')
    parser_type: ParserType | None = Field(default=None, description="Parser type: 'pydantic', 'pydantic_tools', 'str', 'json', or 'custom'")
    output_field_name: str | None = Field(default=None, description='Custom name for the primary output field in schema')
    output_key: str | None = Field(default=None, description='Custom key for output when needed')
    tool_kwargs: dict[str, dict[str, Any]] = Field(default_factory=dict, description='Parameters for tool instantiation')
    bind_tools_kwargs: dict[str, Any] = Field(default_factory=dict, description='Parameters for binding tools to the LLM')
    bind_tools_config: dict[str, Any] = Field(default_factory=dict, description='Configuration for bind_tools')
    preprocess: Callable[[Any], Any] | None = Field(default=None, description='Function to preprocess input before sending to LLM', exclude=True)
    postprocess: Callable[[Any], Any] | None = Field(default=None, description='Function to postprocess output from LLM', exclude=True)
    temperature: float | None = Field(default=None, description='Temperature parameter for the LLM')
    max_tokens: int | None = Field(default=None, description='Maximum number of tokens to generate')
    runtime_options: dict[str, Any] = Field(default_factory=dict, description='Additional runtime options for the LLM')
    custom_runnables: list[Runnable] | None = Field(default=None, description='Custom runnables to add to the chain', exclude=True)
    partial_variables: dict[str, Any] = Field(default_factory=dict, description='Partial variables for the prompt template')
    optional_variables: list[str] = Field(default_factory=list, description='Optional variables for the prompt template')
    uses_messages_field: bool | None = Field(default=None, description='Explicitly specify if this engine uses a messages field. If None, auto-detected.')
    _computed_input_fields: dict[str, tuple[type, Any]] = PrivateAttr(default_factory=dict)
    _computed_output_fields: dict[str, tuple[type, Any]] = PrivateAttr(default_factory=dict)
        ...
        if self.structured_output_model:
            for tool_name, route in self.tool_routes.items():
                if route == 'pydantic_model':
                    ...
                    metadata['is_structured_output'] = tool_name == self.structured_output_model.__name__
    ...
    def _debug_initialization_summary(self):
        ...
        llm_info.add(f'Model: [yellow]{getattr(self.llm_config, 'model', 'Unknown')}[/yellow]')
        ...
        output_info.add(f'Structured Output Model: [yellow]{(self.structured_output_model.__name__ if self.structured_output_model else 'None')}[/yellow]')
        output_info.add(f'Structured Output Version: [yellow]{self.structured_output_version or 'None'}[/yellow]')
        output_info.add(f'Parser Type: [yellow]{self.parser_type or 'None'}[/yellow]')
        console.print(Panel(tree, title='Initialization Complete', border_style='green'))
        ...
        if isinstance(v, dict):
            try:
                ...
                if 'lc' in v and 'type' in v and (v.get('type') == 'constructor'):
                    logger.debug('Attempting to reconstruct prompt template using LangChain load')
                    reconstructed = load(v)
                    if isinstance(reconstructed, BasePromptTemplate):
                        logger.debug(f'Successfully reconstructed: {type(reconstructed)}')
                        return reconstructed
            except Exception as e:
                logger.debug(f'LangChain load failed: {e}, falling back to default template')
                ...
                try:
                    ...
                    for msg_data in v.get('messages', []):
                        if isinstance(msg_data, dict):
                            ...
                            if msg_type == 'system':
                                messages.append(('system', msg_data.get('prompt', {}).get('template', 'You are a helpful assistant.')))
                            elif msg_type == 'human':
                                messages.append(('human', msg_data.get('prompt', {}).get('template', '{input}')))
    ...
    def comprehensive_validation_and_setup(self) -> Self:
        ...
        try:
            debug_print('🔍 [bold blue]Starting comprehensive validation and setup[/bold blue]')
            ...
            debug_print('✅ [bold green]Comprehensive validation and setup complete[/bold green]')
        except Exception as e:
    ...
    def _analyze_tool(self, tool: Any) -> tuple[str, dict[str, Any] | None]:
        ...
        if self.structured_output_model and tool == self.structured_output_model:
            ...
            metadata = {'purpose': 'structured_output', 'version': self.structured_output_version, 'force_choice': self.structured_output_version == 'v2', 'class_name': tool.__name__ if hasattr(tool, '__name__') else str(tool)}
    ...
    def _process_and_validate_tools(self):
        ...
        if not self.tools:
            ...
            debug_print('📝 [yellow]No tools provided - cleared tool-related fields[/yellow]')
            return
        basemodel_tools = []
        tool_names = []
        tool_name_mapping = {}
        for i, tool in enumerate(self.tools):
            ...
            if isinstance(tool, type) and issubclass(tool, BaseModel):
                ...
                if tool not in self.pydantic_tools:
                    ...
                    debug_print(f'➕ [green]Added BaseModel {tool.__name__} to pydantic_tools[/green]')
            if tool_name:
        ...
        self.pydantic_tools = [tool for tool in self.pydantic_tools if tool in current_basemodel_tools]
        ...
        debug_print(f'📊 [cyan]Tool processing complete: {len(tool_names)} tools, {len(basemodel_tools)} BaseModel tools[/cyan]')
    ...
    def _setup_format_instructions(self):
        ...
        if not should_setup:
            debug_print('🚫 [yellow]Format instructions not needed - conditions not met[/yellow]')
            return
        try:
            debug_print(f'📋 [green]Setting up format instructions for: {self.structured_output_model.__name__}[/green]')
            ...
            debug_print('✅ [green]Format instructions added using PydanticOutputParser[/green]')
        except Exception as e:
    ...
    def _should_setup_format_instructions(self) -> bool:
        ...
        if not self.include_format_instructions:
            ...
            return False
        if 'format_instructions' in self.partial_variables:
            debug_print('❌ [yellow]format_instructions already exists in partial_variables[/yellow]')
            return False
    ...
    def _setup_output_handling(self):
        ...
        if self.structured_output_model and self.structured_output_version == 'v2':
            debug_print('[cyan]V2 structured output: NO PARSER (tool-based approach)[/cyan]')
            self.output_parser = None
            self.parser_type = None
            debug_print('[green]V2 mode: output_parser and parser_type set to None[/green]')
            return
        ...
        if self.pydantic_tools and (not self.structured_output_model):
            debug_print('🔧 [cyan]Pydantic tools detected but no structured output model[/cyan]')
            if self.parser_type == 'pydantic_tools':
                self.output_parser = PydanticToolsParser(tools=self.pydantic_tools)
                debug_print('[green]Created PydanticToolsParser for explicit pydantic tools[/green]')
            else:
                debug_print('🎯 [cyan]No automatic parser set for pydantic tools - user must specify parser_type[/cyan]')
        else:
    ...
    def _setup_v2_structured_output(self):
        ...
        debug_print(f'🔧 [cyan]Setting up v2 approach (tool + format instructions, NO PARSER) with {self.structured_output_model.__name__}[/cyan]')
        if self.structured_output_model not in self.tools:
            ...
            debug_print(f'➕ [green]Added {self.structured_output_model.__name__} to tools[/green]')
        if self.structured_output_model not in self.pydantic_tools:
            self.pydantic_tools.append(self.structured_output_model)
        self.parser_type = None
        self.output_parser = None
        debug_print('🎯 [green]V2 mode: NO PARSER - tool forcing with format instructions only[/green]')
        ...
        debug_print(f"🎯 [green]Set force_tool_choice to '{actual_tool_name}' (exact class name)[/green]")
        if self.include_format_instructions:
            try:
                ...
                self.partial_variables['format_instructions'] = instructions
                self._format_instructions_text = instructions
                debug_print('✅ [green]Added format instructions from PydanticOutputParser for structured output model[/green]')
            except Exception as e:
                debug_print(f'❌ [red]Error setting up format instructions: {e}[/red]')
    ...
    def _setup_v1_structured_output(self):
        ...
        debug_print(f'📋 [cyan]Setting up v1 structured output with {self.structured_output_model.__name__}[/cyan]')
        self.parser_type = 'pydantic'
        self.output_parser = PydanticOutputParser(pydantic_object=self.structured_output_model)
    ...
    def _configure_tool_choice(self):
        ...
        if isinstance(self.force_tool_choice, bool):
            if self.force_tool_choice:
                ...
                self.force_tool_use = False
                self.force_tool_choice = None
            debug_print(f'🔄 [yellow]Converted boolean force_tool_choice to mode: {self.tool_choice_mode}[/yellow]')
        elif isinstance(self.force_tool_choice, str):
            self.force_tool_use = True
            self.tool_choice_mode = 'required'
            actual_tool_names = list(self._tool_name_mapping.values())
            if self.force_tool_choice not in actual_tool_names and self.force_tool_choice not in tool_names:
                debug_print(f"⚠️ [yellow]Warning: force_tool_choice '{self.force_tool_choice}' not in available tools: {actual_tool_names}[/yellow]")
        elif isinstance(self.force_tool_choice, list | tuple) and self.force_tool_choice:
            self.force_tool_use = True
            self.tool_choice_mode = 'required'
            self.force_tool_choice = self.force_tool_choice[0]
            debug_print(f'📝 [yellow]Multiple forced tools not supported - using first: {self.force_tool_choice}[/yellow]')
            ...
            if len(tool_names) == 1:
                ...
                debug_print(f'🎯 [green]Auto-selected single tool: {self.force_tool_choice}[/green]')
    ...
    def _update_bind_tools_kwargs(self):
        ...
        if self.tool_choice_mode == 'required':
            if self.force_tool_choice:
                self.bind_tools_kwargs['tool_choice'] = {'type': 'function', 'function': {'name': self.force_tool_choice}}
            else:
    ...
    def _update_bind_tools_kwargs_for_v2(self):
        ...
        if self.force_tool_choice:
            self.bind_tools_kwargs['tool_choice'] = {'type': 'function', 'function': {'name': self.force_tool_choice}}
            debug_print(f"🔧 [green]Set bind_tools_kwargs for v2: forcing tool '{self.force_tool_choice}'[/green]")
        else:
            self.bind_tools_kwargs['tool_choice'] = 'required'
            debug_print("🔧 [yellow]Set bind_tools_kwargs for v2: 'required' (no specific tool)[/yellow]")
    ...
    def _create_prompt_template_if_needed(self):
        ...
        if self.examples and self.example_prompt and self.prefix and self.suffix:
            ...
            debug_print('🤖 [green]Creating ChatPromptTemplate from system message[/green]')
    ...
    def _ensure_messages_placeholder_handling(self):
        ...
        if isinstance(self.prompt_template, ChatPromptTemplate):
            debug_print('💬 [blue]Handling messages placeholder for ChatPromptTemplate[/blue]')
            self._handle_chat_template_messages_placeholder()
        elif isinstance(self.prompt_template, FewShotChatMessagePromptTemplate):
            debug_print('📚 [blue]FewShotChatMessagePromptTemplate detected - messages enabled[/blue]')
            self.uses_messages_field = True
        elif isinstance(self.prompt_template, FewShotPromptTemplate):
            debug_print('📝 [blue]FewShotPromptTemplate detected - messages not applicable[/blue]')
    ...
    def _handle_chat_template_messages_placeholder(self):
        ...
        for i, msg in enumerate(messages):
            if isinstance(msg, MessagesPlaceholder) and getattr(msg, 'variable_name', '') == self.messages_placeholder_name:
        ...
        if not has_messages_placeholder and self.add_messages_placeholder:
            should_be_optional = self.force_messages_optional or self.messages_placeholder_name in self.optional_variables
            new_placeholder = MessagesPlaceholder(variable_name=self.messages_placeholder_name, optional=should_be_optional)
            messages.append(new_placeholder)
            self._update_chat_template_messages(messages)
            debug_print(f'➕ [green]Added messages placeholder (optional={should_be_optional})[/green]')
        elif has_messages_placeholder:
            placeholder = messages[messages_placeholder_index]
            should_be_optional = self.force_messages_optional or self.messages_placeholder_name in self.optional_variables
            if hasattr(placeholder, 'optional') and placeholder.optional != should_be_optional:
                messages[messages_placeholder_index] = MessagesPlaceholder(variable_name=self.messages_placeholder_name, optional=should_be_optional)
                self._update_chat_template_messages(messages)
                debug_print(f'🔄 [cyan]Updated messages placeholder optional={should_be_optional}[/cyan]')
    ...
    def _create_default_chat_template(self):
        ...
        should_be_optional = self.force_messages_optional or self.messages_placeholder_name in self.optional_variables
        messages = [MessagesPlaceholder(variable_name=self.messages_placeholder_name, optional=should_be_optional)]
        self.prompt_template = ChatPromptTemplate.from_messages(messages)
        self.uses_messages_field = True
    ...
    def _create_chat_template_from_system(self):
        ...
        if self.add_messages_placeholder:
            should_be_optional = self.force_messages_optional or self.messages_placeholder_name in self.optional_variables
            messages.append(MessagesPlaceholder(variable_name=self.messages_placeholder_name, optional=should_be_optional))
        self.prompt_template = ChatPromptTemplate.from_messages(messages)
        self.uses_messages_field = True

    def _create_few_shot_template(self):
        """Create a FewShotPromptTemplate from examples and example_prompt."""
        self.prompt_template = FewShotPromptTemplate(examples=self.examples, example_prompt=self.example_prompt, prefix=self.prefix, suffix=self.suffix, input_variables=self.input_variables or [], example_separator=self.example_separator, partial_variables=self.partial_variables, optional_variables=self.optional_variables)
        self.uses_messages_field = False
    ...
    def _create_few_shot_chat_template(self):
        ...
        few_shot_prompt = FewShotChatMessagePromptTemplate(examples=self.examples, example_prompt=self.example_prompt)
    ...
    def _apply_partial_variables(self):
        ...
        try:
            if hasattr(self.prompt_template, 'partial'):
                ...
                debug_print(f'✅ [green]Applied {len(self.partial_variables)} partial variables[/green]')
        except Exception as e:
    ...
    def _apply_optional_variables(self):
        ...
        if isinstance(self.prompt_template, ChatPromptTemplate):
            ...
            if not hasattr(self.prompt_template.optional_variables, 'extend'):
                self.prompt_template.optional_variables = list(self.prompt_template.optional_variables)
    ...
    def _detect_uses_messages_field(self) -> bool:
        ...
        if self.prompt_template:
            if isinstance(self.prompt_template, ChatPromptTemplate | FewShotChatMessagePromptTemplate):
                return True
            if isinstance(self.prompt_template, FewShotPromptTemplate):
                return False
            if hasattr(self.prompt_template, 'input_variables'):
                return self.messages_placeholder_name in self.prompt_template.input_variables
        return True
    ...
    def _compute_schema_fields(self):
        ...
        debug_print(f'📥 [cyan]Input fields: {list(self._computed_input_fields.keys())}[/cyan]')
        debug_print(f'📤 [cyan]Output fields: {list(self._computed_output_fields.keys())}[/cyan]')
    ...
    def _compute_input_fields(self) -> dict[str, tuple[type, Any]]:
        ...
        if self.uses_messages_field:
            is_optional = self.force_messages_optional or self.messages_placeholder_name in self.optional_variables
            if is_optional:
                fields[self.messages_placeholder_name] = (OptionalType[list[BaseMessage]], Field(default_factory=list))
            else:
                fields[self.messages_placeholder_name] = (list[BaseMessage], Field(default_factory=list))
        for var in required_vars:
            if var != self.messages_placeholder_name and var not in fields and (var not in partial_vars):
        ...
        for var, default_value in partial_vars.items():
            if var != self.messages_placeholder_name and var not in fields and (var != 'format_instructions'):
    ...
    def _compute_output_fields(self) -> dict[str, tuple[type, Any]]:
        ...
        if self.structured_output_version == 'v2':
            ...
            return fields
        if self.output_parser:
            if self.parser_type == 'str' or isinstance(self.output_parser, StrOutputParser):
    ...
    def _get_input_variables(self) -> set[str]:
        ...
        all_vars = set()
        if not self.prompt_template:
            return {self.messages_placeholder_name} if self.uses_messages_field else set()
        if hasattr(self.prompt_template, 'input_variables'):
        ...
        if isinstance(self.prompt_template, ChatPromptTemplate):
            for msg in self.prompt_template.messages:
                ...
                if hasattr(msg, 'variable_name'):
                    ...
                    if not getattr(msg, 'optional', False) or not self.force_messages_optional:
                        all_vars.add(var_name)
        partial_vars = set(self.partial_variables.keys())
        if hasattr(self.prompt_template, 'partial_variables'):
            partial_vars.update(getattr(self.prompt_template, 'partial_variables', {}).keys())
        all_vars = all_vars - partial_vars - set(self.optional_variables)
        return all_vars

    def _format_model_schema(self, model_name: str, schema: dict[str, Any], as_section: bool=False) -> str:
    ...
    def _final_validation_check(self):
        ...
        if self.structured_output_model and self.structured_output_version:
            if self.structured_output_model not in self.pydantic_tools:
                debug_print('⚠️ [yellow]Structured output model not in pydantic_tools - adding[/yellow]')
                self.pydantic_tools.append(self.structured_output_model)
        if self.force_tool_choice and (not self.tools):
            debug_print('⚠️ [yellow]force_tool_choice set but no tools available[/yellow]')
            self.force_tool_choice = None
            self.force_tool_use = False
        if self.structured_output_version == 'v2':
            if self.output_parser is not None:
                debug_print('🔧 [yellow]V2 detected: clearing output_parser (should be None)[/yellow]')
                self.output_parser = None
            if self.parser_type is not None:
                debug_print('🔧 [yellow]V2 detected: clearing parser_type (should be None)[/yellow]')
                self.parser_type = None
            if self.structured_output_model:
                if self.structured_output_model not in self.tools:
                    self.tools = list(self.tools) if self.tools else []
                    self.tools.append(self.structured_output_model)
                    debug_print('🔧 [green]Added structured_output_model to tools for v2[/green]')
                ...
                if self.force_tool_choice != expected_tool_name:
                    ...
                    debug_print(f"🔧 [green]Set force_tool_choice to '{expected_tool_name}' for v2[/green]")
                if self.tool_choice_mode != 'required':
                    self.tool_choice_mode = 'required'
                    debug_print("🔧 [green]Set tool_choice_mode to 'required' for v2[/green]")
            ...
            if self.output_parser is None and self.parser_type != 'pydantic':
                debug_print('🔧 [yellow]V1 detected: setting up PydanticOutputParser[/yellow]')
                self.output_parser = PydanticOutputParser(pydantic_object=self.structured_output_model)
                self.parser_type = 'pydantic'
    ...
    def _debug_final_configuration(self):
        ...
        tools_section.add(f'Pydantic Tools: [yellow]{len(self.pydantic_tools)}[/yellow]')
        tools_section.add(f'Tool Routes: [yellow]{len(self.tool_routes)}[/yellow]')
        tools_section.add(f'Tool Is BaseModel: [yellow]{self.tool_is_base_model}[/yellow]')
        ...
        tools_section.add(f'Force Tool Choice: [yellow]{self.force_tool_choice}[/yellow]')
        output_section = tree.add('📤 [cyan]Output Configuration[/cyan]')
        output_section.add(f'Structured Output Model: [yellow]{(self.structured_output_model.__name__ if self.structured_output_model else 'None')}[/yellow]')
        output_section.add(f'Structured Output Version: [yellow]{self.structured_output_version or 'None'}[/yellow]')
        output_section.add(f'Parser Type: [yellow]{self.parser_type or 'None'}[/yellow]')
        output_section.add(f'Format Instructions: [yellow]{('Set' if self._format_instructions_text else 'None')}[/yellow]')
        schema_section = tree.add('📊 [cyan]Schema Configuration[/cyan]')
        schema_section.add(f'Uses Messages Field: [yellow]{self.uses_messages_field}[/yellow]')
        schema_section.add(f'Input Fields: [yellow]{len(self._computed_input_fields)}[/yellow]')
        schema_section.add(f'Output Fields: [yellow]{len(self._computed_output_fields)}[/yellow]')
    ...
    def _get_prompt_template_info(self) -> str:
        ...
        if isinstance(self.prompt_template, ChatPromptTemplate):
            ...
            has_placeholder = any((isinstance(msg, MessagesPlaceholder) and getattr(msg, 'variable_name', '') == self.messages_placeholder_name for msg in self.prompt_template.messages))
    ...
    def apply_runnable_config(self, runnable_config: RunnableConfig | None=None) -> dict[str, Any]:
        ...
        if runnable_config and 'configurable' in runnable_config:
            ...
            aug_llm_params = ['tools', 'force_tool_choice', 'force_tool_use', 'tool_choice_mode', 'temperature', 'max_tokens', 'system_message', 'partial_variables', 'parse_raw_output', 'messages_placeholder_name', 'optional_variables', 'include_format_instructions', 'parser_type', 'pydantic_tools', 'add_messages_placeholder', 'force_messages_optional', 'use_tool_for_format_instructions', 'structured_output_version', 'output_field_name', 'output_key']
    ...
    def _process_input(self, input_data: str | dict[str, Any] | list[BaseMessage]) -> dict[str, Any]:
        ...
        if isinstance(input_data, str):
            result = {}
            if self.uses_messages_field and self.messages_placeholder_name not in result:
                result[self.messages_placeholder_name] = [HumanMessage(content=input_data)]
    ...
    def get_format_instructions(self, model: type[BaseModel] | None=None, as_tools: bool=False) -> str:
            ...
            if self.structured_output_model:
                ...
                if as_tools:
                    ...
                    for tool_model in self.pydantic_tools:
                        ...
                        tool_instructions.append(self._format_model_schema(model_name, schema, as_section=True))
                    if len(tool_instructions) == 1:
                        return tool_instructions[0]
                    return 'You must respond using one of the following formats:\n\n' + '\n\n'.join(tool_instructions)
                target_model = self.pydantic_tools[0]
            elif len(self.tools) == 1 and isinstance(self.tools[0], type) and issubclass(self.tools[0], BaseModel):
    ...
    def add_format_instructions(self, model: type[BaseModel] | None=None, as_tools: bool=False, var_name: str='format_instructions') -> AugLLMConfig:
    ...
    def remove_message(self, index: int) -> AugLLMConfig:
        ...
        if index < len(self.prompt_template.messages):
            ...
            self._update_chat_template_messages(new_messages)
            if isinstance(removed, MessagesPlaceholder) and removed.variable_name == self.messages_placeholder_name:
                if self.add_messages_placeholder:
                    self._ensure_messages_placeholder_handling()
                else:
                    self.uses_messages_field = self._detect_uses_messages_field()
    ...
    def with_structured_output(self, model: type[BaseModel], include_instructions: bool=True, version: str='v2') -> AugLLMConfig:
        """Configure with Pydantic structured output."""
        debug_print(f'[blue]Configuring with structured output (version {version})[/blue]')
        self.structured_output_model = model
        self.include_format_instructions = include_instructions
        if version not in ['v1', 'v2']:
            debug_print(f"[yellow]Invalid version '{version}' - defaulting to 'v2'[/yellow]")
            version = 'v2'
    ...
    def with_pydantic_tools(self, tool_models: list[type[BaseModel]], include_instructions: bool=True, force_use: bool=False) -> AugLLMConfig:
    ...
    def with_format_instructions(self, model: type[BaseModel], as_tool: bool=False, var_name: str='format_instructions') -> AugLLMConfig:
    ...
    def with_tools(self, tools: list[type[BaseTool] | type[BaseModel] | Callable | StructuredTool | BaseTool | str], force_use: bool=False, specific_tool: str | None=None) -> AugLLMConfig:
    ...
    def add_prompt_template(self, prompt_template: BasePromptTemplate) -> AugLLMConfig:
        ...
        debug_print(f'[blue]Adding prompt template: {type(prompt_template).__name__}[/blue]')
        self.prompt_template = prompt_template
        if not self._is_processing_validation:
            self.comprehensive_validation_and_setup()
        debug_print(f'[green]Added prompt template: {type(prompt_template).__name__}[/green]')
        return self

    def add_tool(self, tool: Any, name: str | None=None, route: str | None=None) -> AugLLMConfig:
        """Add a single tool with optional name and route."""
        if tool not in self.tools:
            self.tools = [*list(self.tools), tool]
            if name or route:
                auto_name = name or (getattr(tool, 'name', None) or getattr(tool, '__name__', f'tool_{len(self.tools)}'))
    ...
    def remove_tool(self, tool: Any) -> AugLLMConfig:
        ...
        if tool in self.tools:
            ...
            debug_print(f'➖ [yellow]Removed tool: {tool_name or type(tool).__name__}[/yellow]')
            self._sync_tool_routes()
            if not self._is_processing_validation:
                self._compute_schema_fields()
        return self
        ...
        try:
            ...
            for field_name, (_field_type, field_default) in input_fields.items():
                if field_default is not None:
                    setattr(LLMInput, field_name, Field(default=field_default, description=f'Input for {field_name}'))
                else:
                    setattr(LLMInput, field_name, Field(description=f'Input for {field_name}'))
            ...
            return StructuredTool.from_function(func=llm_function, name=name, description=description, args_schema=LLMInput, **kwargs)
        except ImportError:
            raise ImportError('langchain_core.tools is required for LLM function tools')

    def _create_structured_output_tool(self, name: str, description: str, **kwargs) -> Any:
        ...
        metadata = {'llm_config': self.name or 'anonymous', 'version': self.structured_output_version, 'tool_type': 'structured_output'}
        self.set_tool_route(name, 'structured_output', metadata)
        return tool_class

    def add_tool_with_route(self, tool: Any, route: str, name: str | None=None, metadata: dict[str, Any] | None=None) -> AugLLMConfig:
        """Add a tool with explicit route and metadata."""
        if tool not in self.tools:
            self.tools = [*list(self.tools), tool]
        tool_name = name or (getattr(tool, 'name', None) or getattr(tool, '__name__', f'tool_{len(self.tools)}'))
        self.set_tool_route(tool_name, route, metadata)
        debug_print(f'➕ [green]Added tool with route: {tool_name} -> {route}[/green]')
        self._sync_tool_routes()
        return self

    def create_tool_from_config(self, config: Any, name: str | None=None, route: str | None=None, **kwargs) -> Any:
        ...
        if not hasattr(config, 'to_tool'):
            raise ValueError(f'Config {type(config).__name__} does not support to_tool conversion')
    ...
    def from_prompt(cls, prompt: BasePromptTemplate, llm_config: LLMConfig | None=None, **kwargs):
        ...
        optional_variables = []
        if hasattr(prompt, 'optional_variables') and getattr(prompt, 'optional_variables', None):
        ...
        if uses_messages is None:
            if isinstance(prompt, ChatPromptTemplate):
                uses_messages = any((isinstance(msg, MessagesPlaceholder) and getattr(msg, 'variable_name', '') == messages_placeholder_name or (hasattr(msg, 'role') and msg.role == 'system') for msg in prompt.messages))
            elif isinstance(prompt, FewShotChatMessagePromptTemplate):
                uses_messages = True
            else:
                uses_messages = False
        config = cls(prompt_template=prompt, llm_config=llm_config, partial_variables=partial_variables, uses_messages_field=uses_messages, messages_placeholder_name=messages_placeholder_name, optional_variables=optional_variables, **kwargs)
    ...
    def from_system_prompt(cls, system_prompt: str, llm_config: LLMConfig | None=None, **kwargs):
        ...
        if add_messages_placeholder:
            messages.append(MessagesPlaceholder(variable_name=messages_placeholder_name, optional=is_optional))
        prompt = ChatPromptTemplate.from_messages(messages)
        return cls(prompt_template=prompt, system_message=system_prompt, llm_config=llm_config, uses_messages_field=True, messages_placeholder_name=messages_placeholder_name, optional_variables=optional_variables, add_messages_placeholder=add_messages_placeholder, **kwargs)

    @classmethod
    def from_few_shot(cls, examples: list[dict[str, Any]], example_prompt: PromptTemplate, prefix: str, suffix: str, input_variables: list[str], llm_config: LLMConfig | None=None, **kwargs):
        ...
        optional_variables = kwargs.pop('optional_variables', [])
        few_shot_prompt = FewShotPromptTemplate(examples=examples, example_prompt=example_prompt, prefix=prefix, suffix=suffix, input_variables=input_variables, example_separator=example_separator, partial_variables=partial_variables, optional_variables=optional_variables)
    ...
    @classmethod
    def from_system_and_few_shot(cls, system_message: str, examples: list[dict[str, Any]], example_prompt: PromptTemplate, prefix: str, suffix: str, input_variables: list[str], llm_config: LLMConfig | None=None, **kwargs):
        """Create with system message and few-shot examples."""
        if llm_config is None:
            llm_config = AzureLLMConfig(model='gpt-4o')
        debug_print('[blue]Creating AugLLMConfig with system message and few-shot examples[/blue]')
        partial_variables = kwargs.pop('partial_variables', {})
        example_separator = kwargs.pop('example_separator', '\n\n')
        optional_variables = kwargs.pop('optional_variables', [])
        enhanced_prefix = f'{system_message}\n\n{prefix}'
        few_shot_prompt = FewShotPromptTemplate(examples=examples, example_prompt=example_prompt, prefix=enhanced_prefix, suffix=suffix, input_variables=input_variables, example_separator=example_separator, partial_variables=partial_variables, optional_variables=optional_variables)
    ...
    @classmethod
    def from_tools(cls, tools: list[BaseTool | type[BaseTool] | str | type[BaseModel]], system_message: str | None=None, llm_config: LLMConfig | None=None, use_tool_for_format_instructions: bool | None=None, force_tool_use: bool=False, **kwargs):
        ...
        if add_messages_placeholder:
            is_optional = messages_placeholder_name in optional_variables
            messages.append(MessagesPlaceholder(variable_name=messages_placeholder_name, optional=is_optional))
        prompt_template = ChatPromptTemplate.from_messages(messages) if messages else None
        instance = cls(tools=tools, prompt_template=prompt_template, system_message=system_message, llm_config=llm_config, uses_messages_field=True, messages_placeholder_name=messages_placeholder_name, optional_variables=optional_variables, add_messages_placeholder=add_messages_placeholder, use_tool_for_format_instructions=use_tool_for_format_instructions, force_tool_use=force_tool_use, force_tool_choice=force_tool_choice, tool_choice_mode=tool_choice_mode, **kwargs)
        return instance

    @classmethod
    def from_pydantic_tools(cls, tool_models: list[type[BaseModel]], system_message: str | None=None, llm_config: LLMConfig | None=None, include_instructions: bool=True, force_tool_use: bool=False, **kwargs):
        ...
        prompt_template = ChatPromptTemplate.from_messages(messages) if messages else None
        partial_variables = kwargs.pop('partial_variables', {})
        instance = cls(pydantic_tools=tool_models, tools=tool_models, prompt_template=prompt_template, system_message=system_message, llm_config=llm_config, uses_messages_field=True, messages_placeholder_name=messages_placeholder_name, partial_variables=partial_variables, include_format_instructions=include_instructions, optional_variables=optional_variables, add_messages_placeholder=add_messages_placeholder, force_tool_use=force_tool_use, force_tool_choice=force_tool_choice, tool_choice_mode=tool_choice_mode, **kwargs)
        if include_instructions:
            instance.add_format_instructions(None, True)
        return instance

    @classmethod
    def from_format_instructions(cls, model: type[BaseModel], system_message: str | None=None, llm_config: LLMConfig | None=None, as_tool: bool=False, var_name: str='format_instructions', **kwargs):
        """Create config with format instructions but without structured output."""
        if llm_config is None:
            llm_config = AzureLLMConfig(model='gpt-4o')
        debug_print(f'[blue]Creating AugLLMConfig with format instructions from {model.__name__}[/blue]')
        config = cls.from_system_prompt(system_message=system_message or '', llm_config=llm_config, **kwargs)
        config.add_format_instructions(model, as_tool, var_name)
        return config

    @classmethod
    def from_structured_output_v1(cls, model: type[BaseModel], system_message: str | None=None, llm_config: LLMConfig | None=None, include_instructions: bool=True, **kwargs):
        """Create with v1 structured output using traditional parsing."""
        if llm_config is None:
            llm_config = AzureLLMConfig(model='gpt-4o')
        debug_print(f'[blue]Creating AugLLMConfig with v1 structured output using {model.__name__}[/blue]')
        ...
        if add_messages_placeholder:
            ...
            is_optional = force_messages_optional or messages_placeholder_name in optional_variables
        ...
        instance = cls(llm_config=llm_config, system_message=system_message, messages_placeholder_name=messages_placeholder_name, add_messages_placeholder=add_messages_placeholder, force_messages_optional=force_messages_optional, **kwargs)
        return instance
    ...
    def from_structured_output_v2(cls, model: type[BaseModel], system_message: str | None=None, llm_config: LLMConfig | None=None, include_instructions: bool=False, output_field_name: str | None=None, **kwargs):
        ...
        if llm_config is None:
            llm_config = AzureLLMConfig(model='gpt-4o')
        debug_print(f'[blue]Creating AugLLMConfig with v2 structured output using {model.__name__}[/blue]')
    ...
    def debug_tool_configuration(self) -> AugLLMConfig:
        ...
        if self.structured_output_model:
            ...
            struct_tree.add(f'Model: [yellow]{self.structured_output_model.__name__}[/yellow]')
            struct_tree.add(f'Version: [yellow]{self.structured_output_version}[/yellow]')
    ...
    def remove_tool(self, tool: Any) -> AugLLMConfig:
        ...
        if self.tools:
            ...
            if tool in current_tools:
                ...
                self.tools = current_tools
                debug_print(f'Removed tool: {getattr(tool, 'name', type(tool).__name__)}')
            else:
                debug_print(f'Tool {getattr(tool, 'name', type(tool).__name__)} not found for removal')
        return self
