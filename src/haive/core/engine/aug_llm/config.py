...
import inspect
import logging
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, Type, TypeVar, Union, get_type_hints, cast
...
class AugLLMConfig(InvokableEngine[Union[str, Dict[str, Any], List[BaseMessage]], Union[BaseMessage, Dict[str, Any]]]):
    ...
    def validate_and_setup(self):
        ...
        if self.prompt_template and self.partial_variables:
            if hasattr(self.prompt_template, "partial_variables"):
                ...
                existing_partials = getattr(self.prompt_template, "partial_variables", {}) or {}
                self.prompt_template.partial_variables = {**existing_partials, **self.partial_variables}
    ...
    def get_output_fields(self) -> Dict[str, Tuple[Type, Any]]:
        ...
        Returns:
            Dictionary mapping field names to (type, default) tuples
        """
        from typing import Optional as OptionalType, Dict, List as ListType
        fields = {}
        ...
        # Handle output parser types
            ...
            elif parser_name == "PydanticOutputParser" and hasattr(self.output_parser, "pydantic_object"):
    ...
    def apply_runnable_config(self, runnable_config: Optional[RunnableConfig] = None) -> Dict[str, Any]:
        ...
        if runnable_config and "configurable" in runnable_config:
            ...
            aug_llm_params = [
                "tools", "force_tool_choice", "temperature", "max_tokens", 
    ...
    @classmethod
    def from_tools(cls, tools: List[Union[BaseTool, Type[BaseTool], str]],
                 system_message: Optional[str] = None,
