...
class ToolEngine(InvokableEngine[Dict[str, Any], Dict[str, Any]]):
    ...
    parallel: bool = Field(default=False, description="Execute tools in parallel when possible")
    auto_route: bool = Field(default=True, description="Auto-route to appropriate tool based on calls")
    messages_key: str = Field(default="messages", description="Key for messages in the state")
    tool_choice: str = Field(default="auto", description="Tool choice strategy: 'auto', 'required', or 'none'")
    return_source: bool = Field(default=True, description="Include source information with tool results")
    
    # Advanced options
    timeout: Optional[float] = Field(default=None, description="Timeout for tool execution in seconds")
    max_iterations: Optional[int] = Field(default=None, description="Maximum number of tool iterations")
    ...
    def _get_all_tools(self) -> List[Union[BaseTool, Tool, StructuredTool]]:
        ...
        if self.tools:
            for tool in self.tools:
                ...
                if isinstance(tool, BaseModel) and not isinstance(tool, (BaseTool, Tool, StructuredTool)):
                    try:
    ...
    def _convert_model_to_tool(self, model: BaseModel) -> Optional[StructuredTool]:
        ...
        if not hasattr(model, "__call__") or not callable(getattr(model, "__call__")):
            return None
            
        # Get call method
        call_method = getattr(model, "__call__")
    ...
    def invoke(self, input_data: Dict[str, Any], runnable_config: Optional[RunnableConfig] = None) -> Dict[str, Any]:
        """
        ...
        if isinstance(v, list):
            ...
            for tk in v:
                if isinstance(tk, BaseToolkit) or hasattr(tk, "get_tools"):
                    ...
                    logger.warning(f"Ignoring invalid toolkit type: {type(tk).__name__}")
