...
class DocumentEngine(
    ...
    config: DocumentEngineConfig = Field(
        default_factory=DocumentEngineConfig,
    ...
    loader_registry: Optional[DocumentLoaderRegistry] = Field(
        default=None,
        description="Document loader registry",
        exclude=True
    )
    ...
    def create_runnable(self, runnable_config: Optional[Dict[str, Any]] = None) -> "DocumentEngine":
    ...
    def _analyze_source(self, source: Union[str, Path, Dict[str, Any]]) -> PathAnalysisResult:
...
from haive.core.engine.document.loaders.base.base import SimpleDocumentLoader
        ...
        return SimpleDocumentLoader(
            ...
            loader_options=doc_input.loader_options
        )
    ...
    def _process_documents(
        ...
        if (self.config.processing_strategy == ProcessingStrategy.PARALLEL and 
    ...
    def _process_documents_parallel(
        ...
        with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
            ...
            future_to_doc = {
                executor.submit(
                    self._process_single_document_safe, 
                    doc, 
                    doc_input, 
                    i
                ): i
