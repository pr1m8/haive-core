...
from .debug_interface import HaiveDebugger, console
...
class HaiveInteractiveUI:
    ...
    def _inspect_component(self, name: str):
        ...
        if isinstance(component, Engine):
            ...
            # Generic inspection
            self.console.print(f"[bold]Component: {name} ({type(component).__name__})[/bold]")
            
            # Get attributes
            attrs = {key: value for key, value in component.__dict__.items() 
    ...
    def _test_component(self, name: str):
        ...
        if isinstance(component, Engine):
            ...
            self.console.print(f"[yellow]Don't know how to test component type: {type(component).__name__}[/yellow]")
    
    def _test_engine(self, engine):
        """Test an engine component."""
        self.console.print(f"[bold]Testing Engine: {engine.name} ({engine.engine_type.value})[/bold]")
        ...
        with self.debugger.timer(f"test_{engine.name}"):
            try:
                ...
                if hasattr(engine, "invoke"):
                    ...
                    self.console.print(f"[bold green]Instantiated: {type(instance).__name__}[/bold green]")
            except Exception as e:
                self.console.print(f"[bold red]Error: {str(e)}[/bold red]")
    ...
    def _create_component(self, command):
        ...
        parts = command.split()
        if len(parts) < 2:
            self.console.print("[red]Invalid command. Usage: create <type> <name> [params...][/red]")
            return
    ...
    def _create_graph(self, name):
        ...
        component_list = Prompt.ask("[bold]Enter comma-separated component names to include[/bold]", default="")
        ...
        if component_list:
            for comp_name in component_list.split(","):
                ...
                if component:
                    ...
                    self.console.print(f"[yellow]Warning: Component '{comp_name}' not found, skipping.[/yellow]")
        
        # Create graph
        try:
            graph = DynamicGraph(name=name, components=components)
            self.components[name] = graph
            self.console.print(f"[green]Created graph '{name}' with {len(components)} components.[/green]")
        except Exception as e:
    ...
    def _handle_graph_command(self, command):
        ...
        if subcommand == "create":
            ...
            if len(parts) < 4:
                self.console.print("[red]Usage: graph add <graph> <node> <engine>[/red]")
                return
            self._add_graph_node(parts[1], parts[2], parts[3])
        elif subcommand == "connect":
            if len(parts) < 4:
                self.console.print("[red]Usage: graph connect <graph> <source> <target>[/red]")
                return
    ...
    def _add_graph_node(self, graph_name, node_name, engine_name):
        ...
        try:
            ...
            self.console.print(f"[green]Added node '{node_name}' to graph '{graph_name}'.[/green]")
        except Exception as e:
    ...
    def _connect_graph_nodes(self, graph_name, source, target):
        ...
        try:
            ...
            self.console.print(f"[green]Connected '{source}' to '{target}' in graph '{graph_name}'.[/green]")
        except Exception as e:
    ...
    def _compile_graph(self, graph_name):
        ...
        try:
            with self.debugger.timer(f"compile_{graph_name}"):
                graph.compile()
            self.console.print(f"[green]Graph '{graph_name}' compiled successfully.[/green]")
        except Exception as e:
            self.console.print(f"[red]Error compiling graph: {str(e)}[/red]")
    ...
    def _handle_config_command(self, command):
        ...
        if subcommand == "create":
            ...
            if len(parts) < 4:
                self.console.print("[red]Usage: config add <config> <key> <value>[/red]")
                return
    ...
    def _add_config_param(self, config_name, key, value_str):
        ...
        try:
            ...
            self.console.print(f"[green]Added parameter '{key}' to config '{config_name}'.[/green]")
        except Exception as e:
    ...
    def _use_config(self, component_name, config_name):
        ...
        try:
            if hasattr(component, "invoke"):
                ...
                self.console.print("[yellow]Component does not support invoke method.[/yellow]")
        except Exception as e:
    ...
    def _benchmark_component(self, command):
        ...
        if len(parts) > 1:
            try:
                ...
                self.console.print("[yellow]Invalid iteration count, using default (3).[/yellow]")
        ...
        if isinstance(component, Engine):
            ...
            self.console.print(f"[yellow]Don't know how to benchmark component type: {type(component).__name__}[/yellow]")
    
    def _benchmark_engine(self, engine, iterations):
        """Benchmark an engine component."""
        self.console.print(f"[bold]Benchmarking Engine: {engine.name} ({engine.engine_type.value})[/bold]")
    ...
    def _load_module(self, module_path):
        ...
        try:
            ...
            self.console.print(f"[green]Found and registered {engines_found} engines.[/green]")
        except Exception as e:
    ...
    def _save_component(self, command):
        ...
        try:
            ...
            self.console.print(f"[green]Component '{component_name}' saved to {file_path}[/green]")
        except Exception as e:
