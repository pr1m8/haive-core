...
class HaiveDebugger:
    ...
    def display_engine(self, engine: Any):
        ...
        if hasattr(engine, "model_fields"):
            # Pydantic v2
            fields = {name: str(field.annotation) for name, field in engine.model_fields.items() 
    ...
    def display_graph(self, graph: Any, visualize: bool = True):
        ...
        layout = Layout()
        layout.split_row(
            Layout(nodes_table, name="nodes"),
            Layout(edges_table, name="edges")
        )
        ...
        if visualize and hasattr(graph, "visualize"):
            try:
                ...
                self.console.print(f"[bold green]Graph visualization saved to {output_file}[/bold green]")
            except Exception as e:
                self.console.print(f"[bold red]Error generating visualization: {str(e)}[/bold red]")
    ...
    def timer(self, name: str):
        ...
        try:
            ...
            self.console.print(f"[bold cyan]Timer[/bold cyan] {name}: {elapsed:.4f} seconds")
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    # Log success
                    self.logger.debug(f"TRACE EXIT: {module_name}.{func_name} (returned {type(result).__name__}) [{elapsed:.4f}s]")
                    ...
                    # Log error
                    self.logger.error(f"TRACE ERROR: {module_name}.{func_name} - {type(e).__name__}: {str(e)} [{elapsed:.4f}s]")
    ...
    def show_trace_summary(self):
        ...
        for trace in self.execution_traces[-20:]:  # Show last 20 traces
            status = "[green]SUCCESS[/green]" if trace["success"] else f"[red]ERROR: {trace.get('error', 'Unknown')}[/red]"
        ...
        for obj in gc.get_objects():
            if id(obj) == obj_hash:
                ...
                if curr_snapshot != prev_snapshot:
                    self.console.print(f"[bold yellow]Changes detected in {obj_name}[/bold yellow]")
                    
                    # If dictionaries, show key differences
                    if isinstance(curr_snapshot, dict) and isinstance(prev_snapshot, dict):
                        # Keys added
                        added = set(curr_snapshot.keys()) - set(prev_snapshot.keys())
                        if added:
                            self.console.print(f"  [green]Added keys: {', '.join(added)}[/green]")
                        
                        # Keys removed
                        removed = set(prev_snapshot.keys()) - set(curr_snapshot.keys())
                        if removed:
                            self.console.print(f"  [red]Removed keys: {', '.join(removed)}[/red]")
                        
                        # Changed values
                        changed = {k: (prev_snapshot[k], curr_snapshot[k]) for k in set(prev_snapshot.keys()) & set(curr_snapshot.keys())
