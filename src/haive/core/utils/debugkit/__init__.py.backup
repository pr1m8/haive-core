...
Examples:
    Quick start with unified interface::
        ...
        # Enhanced debugging
        dev.ice("Debug variable", value=42)

        # Rich logging with context
        with dev.context("operation") as ctx:
    ...
    Advanced analysis::
        ...
        analysis = dev.analyze_code(my_function)
        ...
        # Run static analysis
        results = dev.static_analysis.analyze_file(Path("module.py"))
        report = dev.static_analysis.generate_report(results)
"""
...
from .config import DevConfig, Environment, LogLevel, StorageBackend, config

if TYPE_CHECKING:
    from .analysis.complexity import ComplexityAnalyzer, ComplexityReport
    from .analysis.static import AnalysisResult, StaticAnalysisOrchestrator
    from .analysis.types import FunctionTypeAnalysis, TypeAnalyzer

# Import with fallbacks for missing dependencies
try:
    from .debugging import debug
except ImportError:
    from .fallbacks import FallbackDebug
...
try:
    ...
    from .fallbacks import FallbackLog
...
try:
    ...
    from .fallbacks import FallbackTrace
...
try:
    ...
    from .fallbacks import FallbackProfile
...
try:
    ...
    from .fallbacks import FallbackBenchmark
...
def get_type_analyzer() -> "TypeAnalyzer":
    ...
    if _type_analyzer is None:
        from .analysis.types import TypeAnalyzer
...
def get_static_orchestrator() -> "StaticAnalysisOrchestrator":
    ...
    if _static_orchestrator is None:
        from .analysis.static import StaticAnalysisOrchestrator
...
class DevContext:
    ...
    Examples:
        Basic context usage::
            ...
            with dev.context("user_registration") as ctx:
        ...
        Nested contexts::
            ...
            with dev.context("api_request") as outer:
                with dev.context("database_query") as inner:
    ...
    def checkpoint(self, checkpoint_name: str, **data: Any) -> None:
        ...
        if config.trace_enabled and hasattr(trace, "mark"):
...
class CodeAnalysisReport:
    ...
    Examples:
        Review comprehensive analysis::
            ...
            report = dev.analyze_code(my_function)
...
class UnifiedDev:
    ...
    Examples:
        Basic usage::
            ...
            # Quick debugging
            dev.ice("Debug info", variable=value)
            dev.info("Process started")
    ...
    def context(self, name: str, **metadata: Any) -> DevContext:
        ...
        Examples:
            Create context::
                ...
                with dev.context("user_registration", user_id=123) as ctx:
    ...
    def analyze_code(self, func: Callable[..., Any]) -> CodeAnalysisReport:
        ...
        Examples:
            Analyze function quality::
                ...
                report = dev.analyze_code(complex_function)
        ...
        **options: Any,
        ...
        Examples:
            ...
            Selective instrumentation::
                ...
                @dev.instrument(analyze=True, profile=True, log=False)
        ...
        def decorator(f: Callable) -> Callable:
            ...
            def wrapper(*args, **kwargs):
                ...
                with self.context(f"function.{func_name}") as ctx:
                    ...
                    # Runtime type checking if enabled
                    if self.config.runtime_type_check and analysis_report:
                        self._validate_runtime_types(
                            f, args, kwargs, analysis_report.type_analysis
                        )
                    ...
                    try:
                        ...
                        # Validate return type if enabled
                        if self.config.runtime_type_check and analysis_report:
                            self._validate_return_type(
                                f, result, analysis_report.type_analysis
                            )
        ...
        # Handle both @dev.instrument and @dev.instrument(...) usage
        if func is None:
            return decorator
        else:
    ...
    def _validate_runtime_types(
        ...
        # Implementation would check args/kwargs against type_analysis.parameters
        # This is a placeholder for the actual validation logic
        pass

    def _validate_return_type(
        self, func: Callable, result: Any, type_analysis: "FunctionTypeAnalysis"
    ) -> None:
        """Validate function return value against type annotation at runtime."""
        # Implementation would check result against type_analysis.return_type
        # This is a placeholder for the actual validation logic
        pass
    ...
    def ice(self, *args, **kwargs) -> Any:
        ...
        Examples:
            Debug variables::
                ...
                result = dev.ice(process_data(input_data))
                dev.ice("Processing", count=len(items), status="active")
        """
    ...
    def clear_cache(self) -> None:
        ...
        Examples:
            Clear caches for fresh analysis::
                ...
                dev.clear_cache()
                fresh_report = dev.analyze_code(my_function)
        """
