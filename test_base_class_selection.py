...
This tests that LLM engines always get LLMState as their base clas."""
...
def test_base_class_selectio():
    """Test that LLM engines always get LLMState as their base clas."""
    ...
    assert base_class == LLMState, f"Expected LLMState, got {base_clas}"

    # Test that LLMState properly inherits the hierarchy

    # Verify the inheritance chain
    assert issubclass(LLMState, ToolState), "LLMState should inherit from ToolStat"
    assert issubclass(
        ToolState, MessagesStateWithTokenUsage
    ), "ToolState should inherit from MessagesStateWithTokenUsag"
    assert issubclass(
        MessagesStateWithTokenUsage, StateSchema
    ), "MessagesStateWithTokenUsage should inherit from StateSchem"
