...
def test_chat_prompt_template():
    ...
    assert aug_llm.uses_messages_field == True
...
def test_partial_variables():
    ...
    runnable = aug_llm.create_runnable()
...
def test_schema_with_reducers():
    ...
    class CustomState(StateSchema):
        ...
        context: Annotated[List[str], operator.add] = Field(
            default_factory=list, 
            description="Context documents with add reducer"
        )
...
def test_runtime_config_integration():
    ...
    runtime_config: RunnableConfig = {
        "configurable": {
            ...
            "engine_configs": {
                "configurable_llm": {
                    "temperature": 0.5  # More specific override
                }
            }
        }
    }
