...
def analyze_pydantic_model_distinction():
    ...
    print("   This model SHOULD go to parse_output route")
...
def demonstrate_capability_based_routing():
    ...
    for tool_name, route in config.tool_routes.items():
        ...
        elif tool_name == config.structured_output_model.__name__:
...
def show_augllmconfig_structured_output_detection():
    ...
    print("   if self.structured_output_model:")
    print("       model_name = self.structured_output_model.__name__")
    print("       self.set_tool_route(model_name, 'parse_output')")
    print("       # Instead of 'pydantic_model'")
