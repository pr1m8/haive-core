...
from haive.core.engine.agent.persistence.postgres_config import PostgresCheckpointerConfig, POSTGRES_AVAILABLE
...
pytestmark = pytest.mark.skipif(not POSTGRES_AVAILABLE, reason="PostgreSQL dependencies not available")
...
class TestPostgresCheckpointerConfig:
    ...
    def test_create_checkpointer(self, test_config):
        ...
        assert checkpointer.__class__.__name__ == "PostgresSaver", "Should create a PostgresSaver instance"
    ...
    def test_pool_reuse(self, test_config):
        ...
        checkpointer1 = test_config.create_checkpointer()
        
        # Store reference to pool
        pool1 = test_config._pool
        
        # Create second checkpointer
        checkpointer2 = test_config.create_checkpointer()
    ...
    def test_auto_commit_setting(self):
        ...
        try:
            # Create checkpointer
            checkpointer_autocommit = config_autocommit.create_checkpointer()
            ...
            with config_autocommit._pool.connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute("SELECT 1 FROM threads WHERE thread_id = %s", (thread_id_autocommit,))
                    result = cursor.fetchone()
        ...
        try:
            # Create checkpointer
            checkpointer_no_autocommit = config_no_autocommit.create_checkpointer()
            ...
            try:
                ...
                try:
                    ...
                    cursor.execute("SELECT 1 FROM threads WHERE thread_id = %s", (thread_id_no_autocommit,))
                    result = cursor.fetchone()
                    ...
                    with config_no_autocommit._pool.connection() as conn2:
                        with conn2.cursor() as cursor2:
                            cursor2.execute("SELECT 1 FROM threads WHERE thread_id = %s", (thread_id_no_autocommit,))
    ...
    def test_list_checkpoints(self, test_config):
        ...
        config2 = {"configurable": {"thread_id": thread_id, "checkpoint_id": f"checkpoint_{uuid.uuid4()}"}}
