...
def test_aug_llm_config_uses_llm_state():
    ...
    messages_index = mro_names.index("MessagesState") if "MessagesState" in mro_names else -1

    if messages_index != -1:
        assert llm_index < messages_index, "LLMState should come before MessagesState in MRO"
    ...
    assert "token_usage" in schema.model_fields, "LLMState should provide token_usage field"
...
def test_schema_composer_detects_llm_engine_early():
    ...
    assert composer.detected_base_class == LLMState, (
