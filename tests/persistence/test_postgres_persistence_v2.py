...
class Test_PostgresCheckpointer:
    ...
    def test_list_checkpoints(self, postgres_config, test_thread_id, test_data):
        ...
        for i in range(3):
            ...
            updated_config = postgres_config.put_checkpoint(config, test_data_copy)
    ...
    def test_delete_thread(self, postgres_config, test_thread_id, test_data):
        ...
        with pool.connection() as conn:
            with conn.cursor() as cursor:
                ...
                assert result is None, f"Checkpoints for thread {test_thread_id} still exist in database"
...
class Test_PostgresShallowCheckpointer:
    ...
    def test_shallow_list_checkpoints(self, postgres_shallow_config, test_thread_id, test_data):
        ...
        for i in range(3):
            test_data_copy = test_data.copy()
            test_data_copy["state"]["_counter"] = i + 1
            updated_config = postgres_shallow_config.put_checkpoint(config, test_data_copy)
        ...
        logger.info(f"✅ Shallow mode correctly retained only the latest checkpoint")
...
class Test_PostgresAsyncCheckpointer:
            ...
            if pool:
                async with pool.connection() as conn:
                    async with conn.cursor() as cursor:
                        await cursor.execute("SELECT 1 FROM threads WHERE thread_id = %s", (test_thread_id,))
    ...
    @pytest.mark.asyncio
    async def test_async_put_and_get_checkpoint(self, postgres_async_config, test_thread_id, test_data):
        ...
        try:
            ...
            logger.info(f"✅ Retrieved checkpoint data asynchronously: {json.dumps(checkpoint, indent=2)}")
...
class Test_CheckpointerSetup:
    ...
    def test_setup_postgres_checkpointer(self, test_thread_id):
        ...
        try:
            ...
            logger.info(f"✅ Set up PostgreSQL checkpointer using utility function")
