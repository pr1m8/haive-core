...
def sample_documents():
    ...
    return [Document(page_content=text, metadata={"source": f"doc{i}"}) for i, text in enumerate(texts)]
...
def test_structured_output_model_fields(azure_llm_config):
    ...
    qa_prompt = ChatPromptTemplate.from_messages([
        ("system", "You are an AI assistant that generates questions and answers from text."),
...
def test_complex_field_mapping(azure_llm_config):
    ...
    node_config = NodeConfig(
        ...
        engine=analysis_llm,
        input_mapping={
            "content": "document"  # Map state.content to engine.document
        },
        output_mapping={
...
def test_schema_composer_includes_all_fields(azure_llm_config, retriever):
    ...
    doc_prompt = ChatPromptTemplate.from_messages([
        ...
        ("human", "Analyze this document and create a structured outline:\n\n{contents}")
    ])
