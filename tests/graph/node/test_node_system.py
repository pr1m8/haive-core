...
from haive.core.engine.retriever import RetrieverConfig, VectorStoreRetrieverConfig
...
def test_async_engine_node(azure_llm_config, node_registry):
    ...
    assert isinstance(result, Command)
    assert result.goto == END
    assert "content" in result.update or "aimessage" in result.update  # Check for either possible key
...
def test_factory_conditional_node_creation(node_registry):
    ...
    logger.info(f"Conditional Node Results - High: {high_result.goto}, Medium: {medium_result.goto}, Low: {low_result.goto}")
...
def test_retriever_node(retriever, node_registry):
    ...
    logger.info(f"Retriever Node Result: {[doc.page_content[:50] for doc in result.update['result']]}")
...
def test_structured_output_model(azure_llm_config, node_registry):
    ...
    analysis_prompt = ChatPromptTemplate.from_messages([
        ...
        ("human", "Analyze the following text and extract key information:\n\n{content}")
    ])
...
def test_complex_node_chain(azure_llm_config, retriever, node_registry):
    ...
    def process_query(state):
        ...
        return {
            ...
            "query_type": "informational" if "what" in query.lower() or "how" in query.lower() else "navigational"
        }
...
def test_integration_with_dynamic_graph(azure_llm_config, retriever, node_registry):
    ...
    graph = DynamicGraph(
        name="rag_workflow",
        components=[retriever, answer_llm]
    )
    ...
    try:
        ...
        logger.info(f"Graph structure verified with {len(graph.nodes)} nodes and {len(graph.edges)} edges")
