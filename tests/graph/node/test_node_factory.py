...
def test_create_node_function_with_real_engine(real_llm_engine):
    ...
    logger.debug(f"Asserted __node_config__.engine is the original engine: {node_function.__node_config__.engine}")
    ...
    logger.debug(f"Asserted __engine_id__ matches engine.id: {node_function.__engine_id__}")
...
def test_node_function_with_mappings(real_llm_engine):
    ...
    assert hasattr(node_function, "__node_config__")
    logger.debug(f"Checking mappings on attached __node_config__: {node_function.__node_config__}")
    assert node_function.__node_config__.input_mapping == input_mapping
    logger.debug(f"Asserted input_mapping on node_config: {node_function.__node_config__.input_mapping}")
    assert node_function.__node_config__.output_mapping == output_mapping
    logger.debug(f"Asserted output_mapping on node_config: {node_function.__node_config__.output_mapping}")
...
def test_config_overrides_in_node_config(real_llm_engine):
    ...
    logger.debug(f"Asserted config_overrides on NodeConfig object: {node_config.config_overrides}")
    ...
    logger.debug(f"Created node function: {node_function}")
    assert hasattr(node_function, "__node_config__")
    logger.debug(f"Checking overrides on attached __node_config__: {node_function.__node_config__}")
    assert node_function.__node_config__.config_overrides == config_overrides
    logger.debug(f"Asserted config_overrides on attached node_config: {node_function.__node_config__.config_overrides}")
...
def test_extract_input():
    ...
    state = {"key1": "value1", "key2": "value2", "runnable_config": {"config_key": "config_val"}}
    ...
    expected_no_map = {"key1": "value1", "key2": "value2"} # runnable_config should be excluded
...
def test_handle_result():
    ...
    logger.debug(f"Testing _handle_result with dict and output mapping: {result_map}, {output_mapping}")
...
def test_process_output():
    ...
    logger.debug(f"Testing _process_output with dict and mapping: {output_dict}, {mapping}")
    ...
    logger.debug(f"Testing _process_output with non-dict and mapping: {output_str}, {mapping}")
