...
def test_state_schema_with_reducers():
    ...
    assert len(state1.messages) == 1, f"Expected 1 message after applying reducer, got {len(state1.messages)}"
    assert state1.messages[0].content == "Test", f"Expected message content to be 'Test', got '{state1.messages[0].content}'"
...
def test_state_schema_to_manager():
    ...
    assert 'new_field' in enhanced_schema.__annotations__, "Field 'new_field' missing from enhanced schema"
    ...
    assert instance.new_field == "new", f"Expected new_field to be 'new', got '{instance.new_field}'"
...
def test_schema_composer_create_model():
    ...
    assert instance1.items == ["item1", "item2", "item3", "item4"], f"Lists not properly combined: {instance1.items}"
    ...
    assert new_instance.messages[0].content == "Hello", f"Expected first message content to be 'Hello'"
    assert new_instance.messages[1].content == "Hi there", f"Expected second message content to be 'Hi there'"
    assert len(new_instance.items) == 4, f"Expected 4 items after deserialization, got {len(new_instance.items)}"
...
def test_state_schema_manager_with_engines():
    ...
    # Verify reducers
    assert "iterations" in enhanced_schema.__serializable_reducers__, "Reducer for 'iterations' not found"
    reducer_name = enhanced_schema.__serializable_reducers__["iterations"]
    logger.info(f"Reducer for iterations: {reducer_name}")
    assert reducer_name == "operator.add", f"Expected reducer name to be 'operator.add', got '{reducer_name}'"
    ...
    # Verify reducer application
    assert state1.iterations == 5, f"Expected iterations to be 5 after adding 2+3, got {state1.iterations}"
...
def test_combined_workflows():
    ...
    assert state.metadata["total_time"] == 0.5, f"Expected total_time to be 0.5, got {state.metadata['total_time']}"
...
def test_complex_schema_with_multiple_reducers():
    ...
    assert state1.token_count == 35, f"Expected token_count to be 35 after adding 10+25, got {state1.token_count}"
    assert len(state1.trace_log) == 3, f"Expected 3 trace log entries, got {len(state1.trace_log)}"
    assert state1.trace_log[0] == "start", f"Expected first log entry to be 'start', got '{state1.trace_log[0]}'"
    assert state1.trace_log[1] == "processing", f"Expected second log entry to be 'processing', got '{state1.trace_log[1]}'"
    ...
    assert state1.stats["queries"] == 1, f"Expected queries stat to be 1, got {state1.stats['queries']}"
    assert state1.stats["tokens"] == 100, f"Expected tokens stat to be 100, got {state1.stats['tokens']}"
...
def test_operator_reducers_comprehensive():
    ...
    assert state1.concat_field == "Hello World", f"Expected concat_field to be 'Hello World', got '{state1.concat_field}'"
    assert state1.max_field == 20, f"Expected max_field to be 20 (max(10,20)), got {state1.max_field}"
    assert state1.min_field == 30, f"Expected min_field to be 30 (min(50,30)), got {state1.min_field}"
    ...
    for field, expected_name in [
        ...
        assert actual_name == expected_name, f"Expected reducer name for {field} to be '{expected_name}', got '{actual_name}'"
...
def test_schema_with_message_reducer():
    ...
    assert state.messages[0].content == "What is AI?", f"Expected first message to be 'What is AI?', got '{state.messages[0].content}'"
    assert state.messages[1].content == "AI is a field of computer science focused on creating intelligent machines.", f"Expected second message content to match"
    assert state.messages[0].type == "human", f"Expected first message type to be 'human', got '{state.messages[0].type}'"
    assert state.messages[1].type == "ai", f"Expected second message type to be 'ai', got '{state.messages[1].type}'"
    ...
    assert len(serialized["messages"]) == 2, f"Expected 2 serialized messages, got {len(serialized['messages'])}"
    ...
    assert len(restored.messages) == 2, f"Expected 2 messages in restored state, got {len(restored.messages)}"
    assert restored.messages[0].content == "What is AI?", f"Expected first message to be 'What is AI?', got '{restored.messages[0].content}'"
...
def test_node_function_creation():
    ...
    assert len(result.update["results"]) == 2, f"Expected 2 results, got {len(result.update['results'])}"
...
def test_schema_manager_field_operations():
    ...
    manager.modify_field(
        "count",
        new_default=10,
        new_description="Modified count field"
    )
...
def test_command_and_send_helpers():
    ...
    # Verify Command
    assert command.update["message"] == "Hello", f"Expected message to be 'Hello', got '{command.update['message']}'"
    ...
    assert send.arg["message"] == "Routed message", f"Expected message to be 'Routed message', got '{send.arg['message']}'"
...
def test_various_reducer_types():
    ...
    assert state1.items == ["a", "b", "c", "d"], f"Expected items to be ['a', 'b', 'c', 'd'], got {state1.items}"
